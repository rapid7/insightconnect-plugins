...
class SalesforceAPI:
    def __init__(self, client_id, client_secret, username, password, security_token, logger):
        self.logger = logger

        self._get_token(client_id, client_secret, username, password, security_token)
        self._get_version()

    def simple_search(self, text):
        return self._call_api("GET", "parameterizedSearch/", params={"q": text})["searchRecords"]
    ...
    def get_record(self, record_id, external_id_field_name, object_name):
        if external_id_field_name:
            url = "sobjects/{}/{}/{}".format(object_name, external_id_field_name, record_id)
        else:
            url = "sobjects/{}/{}".format(object_name, record_id)
    ...
    def _get_token(self, client_id, client_secret, username, password, security_token):
