    ...
    def validateIPAddressRange(self, host_range):
        ...
        for index, value in enumerate(breakdown):
            if index != (len(breakdown) - 1) and "-" in value:
                raise ValueError(
                    "Error creating target: IP address range is not in correct format"
    ...
    def run(self, params={}):
        ...
        try:
            ...
            return {
                "target_id": "",
                "success": False,
                "message": "Error  trying to create target, IP not valid: " + str(err),
            }

        try:
            target_id = self.connection.scanner.create_target(
                name=target_name, hosts=host_list, comment="", port_list=target_port_list_id
            )
        ...
        return {
            "target_id": target_id,
            "success": True,
            "message": "Successfully created target in the OpenVAS server",
        }
