    ...
    def run(self, params={}):
        ...
        try:
            ...
            return {
                "schedule_id": "",
                "success": False,
                "message": "Error trying to parse First Time datetime object: " + str(err),
            }

        try:
            if duration and period:
                schedule_id = self.connection.scanner.create_schedule(
                    name=name,
                    hour=first_time_hour,
                    minute=first_time_minute,
                    month=first_time_month,
                    day=first_time_day,
                    year=first_time_year,
                    period=period,
                    duration=duration,
                )
