...
class CheckIfAddressInAddressGroup(insightconnect_plugin_runtime.Action):
    ...
    def run(self, params={}):
        ...
        for ip in ["ipv4", "ipv6"]:
            address_object = (
                address_group.get("address_group", {}).get(ip, {}).get("address_object", {})
            )
        ...
        if len(objects_matching) == 0 and params.get(Input.ENABLE_SEARCH) is True:
            for object_from_group in address_objects_names:
                response = self.connection.sonicwall_api.get_address_object(
                    object_from_group.get("name")
                )
                object_type = response.get("object_type", {})
                address_object = (
                    response.get("address_object", {}).get("address_object").get(object_type)
                )
