...
class HippocampeAPI:
    ...
    def distinct(self, intelligence_types):
        return self._call_api("POST", "distinct", json={"field": intelligence_types})
    ...
    def hipposcore(self, observables):
        ...
        raw_scores = self._call_api("POST", "hipposcore", json=parsed_observables)
        scores = []
    ...
    def more(self, observables):
        ...
        raw_results = self._call_api("POST", "more", json=parsed_observables)
        results = []
        ...
        for observable, values in raw_results.items():
            ...
            for value in values:
                if "hipposcore" in value:
                    value["hipposcore"] = float(value["hipposcore"]["hipposcore"])
    ...
    def shadowbook(self):
        ...
        job = {"source": source, "status": status}
            ...
            if response.status_code == 401:
                ...
                if cause:
                    raise ConnectionTestException(
                        cause=cause, assistance=("If the issue persists please contact support.")
                    )
