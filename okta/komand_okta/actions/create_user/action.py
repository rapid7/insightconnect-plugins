...
class CreateUser(komand.Action):
    ...
    def run(self, params={}):
        ...
        url_params = {
            ...
            "activate": str(params.get('activate', True)).lower(),
            "provider": str(params.get('provider', False)).lower(),
            "nextLogin": str(params.get('nextLogin', False)).lower()
        }
                                    ...
                                    for error in data['errorCauses']:
                                        if 'errorSummary' in error:
                                            if 'An object with this field already exists' in error['errorSummary']:
                                                raise Exception('Okta: Create user failed. The user may already exist')
        ...
        raise Exception(
                    'Okta: Create user failed unexpectedly.'
                    ' Make sure any provided objects, such as profile or credentials match the Okta schemas.'
    ...
    def build_post(self, params):
        ...
        """
        data = {}
        data['profile'] = params.get('profile')
        # "credentials": {
        #   "password": {
        ...
        if 'credentials' in params:
            data['credentials'] = params.get('credentials')
            # If some of these are empty we must remove them otherwise API will complain
            if not params.get('credentials').get('password').get('value'):
                    del data['credentials']['password']
            if not params.get('credentials').get('recovery_question').get('answer'):
                    del data['credentials']['recovery_question']
            # This may no longer be here from above
            if 'recovery_question' in params.get('credentials'):
                if not params.get('credentials').get('recovery_question').get('question'):
                        del data['credentials']['recovery_question']
            if not params.get('credentials').get('provider').get('name'):
                    del data['credentials']['provider']
            # This may no longer be here from above
            if 'provider' in params.get('credentials'):
                if not params.get('credentials').get('provider').get('type'):
                        del data['credentials']['provider']

        if 'groupIds' in params:
            data['groupIds'] = params.get('groupIds')
