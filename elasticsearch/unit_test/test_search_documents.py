...
from komand_elasticsearch.connection.connection import Connection
from komand_elasticsearch.actions.search_documents import SearchDocuments
import json
import logging


class TestSearchDocuments(TestCase):
    def test_integration_search_documents(self):
        """
        ...
        self.assertTrue("hits" in results.keys())
        self.assertTrue("timed_out" in results.keys())
        self.assertTrue("_shards" in results.keys())
        self.assertTrue("took" in results.keys())
        self.assertIsNotNone(results.get("hits"))
