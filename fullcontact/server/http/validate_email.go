...
// ValidateEmailHandler is a handler
type ValidateEmailHandler struct {
	c *connection.Cache
}

// NewValidateEmailHandler returns a new ValidateEmailHandler
func NewValidateEmailHandler(c *connection.Cache) *ValidateEmailHandler {
...
func (h *ValidateEmailHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ValidateEmailInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ValidateEmailInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ValidateEmailAction{}
	var output *actions.ValidateEmailOutput
	if isTest {
...
func (h *ValidateEmailHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
