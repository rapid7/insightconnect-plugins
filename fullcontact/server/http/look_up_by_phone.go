...
// LookUpByPhoneHandler is a handler
type LookUpByPhoneHandler struct {
	c *connection.Cache
}

// NewLookUpByPhoneHandler returns a new LookUpByPhoneHandler
func NewLookUpByPhoneHandler(c *connection.Cache) *LookUpByPhoneHandler {
	return &LookUpByPhoneHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *LookUpByPhoneHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LookUpByPhoneInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LookUpByPhoneInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.LookUpByPhoneAction{}
	var output *actions.LookUpByPhoneOutput
	if isTest {
...
func (h *LookUpByPhoneHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
