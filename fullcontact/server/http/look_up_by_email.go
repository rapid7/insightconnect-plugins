...
// LookUpByEmailHandler is a handler
type LookUpByEmailHandler struct {
	c *connection.Cache
}

// NewLookUpByEmailHandler returns a new LookUpByEmailHandler
func NewLookUpByEmailHandler(c *connection.Cache) *LookUpByEmailHandler {
	return &LookUpByEmailHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *LookUpByEmailHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LookUpByEmailInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LookUpByEmailInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.LookUpByEmailAction{}
	var output *actions.LookUpByEmailOutput
	if isTest {
...
func (h *LookUpByEmailHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
