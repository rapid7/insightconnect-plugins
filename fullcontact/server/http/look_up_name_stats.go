...
// LookUpNameStatsHandler is a handler
type LookUpNameStatsHandler struct {
	c *connection.Cache
}

// NewLookUpNameStatsHandler returns a new LookUpNameStatsHandler
func NewLookUpNameStatsHandler(c *connection.Cache) *LookUpNameStatsHandler {
	return &LookUpNameStatsHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *LookUpNameStatsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LookUpNameStatsInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LookUpNameStatsInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.LookUpNameStatsAction{}
	var output *actions.LookUpNameStatsOutput
	if isTest {
...
func (h *LookUpNameStatsHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
