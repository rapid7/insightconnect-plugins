...
func (i *LookUpByPhoneInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *LookUpByPhoneOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *LookUpByPhoneAction) Run(conn *connection.Connection, input *LookUpByPhoneInput, log plog.Logger) (*LookUpByPhoneOutput, error) {
...
func (a *LookUpByPhoneAction) Test(conn *connection.Connection, input *LookUpByPhoneInput, log plog.Logger) (*LookUpByPhoneOutput, error) {
	output := LookUpByPhoneOutput{}

	params := implementationParams{
		ServerBase:     conn.Server,
		Service:        "person.json",
		URLKeys:        []string{"phone"},
		URLValues:      []string{input.Phone},
		ApplicationKey: conn.Token.SecretKey,
		TimeoutInSec:   10,
	}
	marshaledJSON := Query(params)
	shouldRetry := Postprocess(marshaledJSON, Receivers{PersonPtr: &output.PersonInfo}, 10) //a.input.RetryInterval)
	if shouldRetry {
		marshaledJSON := Query(params)
