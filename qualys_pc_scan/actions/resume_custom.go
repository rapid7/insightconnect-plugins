...
import (
	...
	"github.com/rapid7/komand-plugins/qualys_pc_scan/connection"
)
...
func (i *ResumeInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *ResumeOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *ResumeAction) Run(conn *connection.Connection, input *ResumeInput, log plog.Logger) (*ResumeOutput, error) {
	pcscans := conn.CustomParams.Qualys.PCScans
	if _, _, err := pcscans.Resume(context.Background(), qualys.PCScanResumeOptions{
		Reference: input.Reference,
	}); err != nil {
		return &ResumeOutput{}, err
	}
	return &ResumeOutput{}, nil
}
...
func (a *ResumeAction) Test(conn *connection.Connection, input *ResumeInput, log plog.Logger) (*ResumeOutput, error) {
	output := &ResumeOutput{}
	// Your code here
