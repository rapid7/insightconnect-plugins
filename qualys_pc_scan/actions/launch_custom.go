...
func (i *LaunchInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *LaunchOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *LaunchAction) Run(conn *connection.Connection, input *LaunchInput, log plog.Logger) (*LaunchOutput, error) {
	pcscans := conn.CustomParams.Qualys.PCScans

	scan, _, err := pcscans.Launch(context.Background(), qualys.PCScanLaunchOptions{
		Title:       input.Title,
...
func (a *LaunchAction) Test(conn *connection.Connection, input *LaunchInput, log plog.Logger) (*LaunchOutput, error) {
	output := &LaunchOutput{}
	// Your code here
