...
// PauseHandler is a handler
type PauseHandler struct {
	c *connection.Cache
}

// NewPauseHandler returns a new PauseHandler
func NewPauseHandler(c *connection.Cache) *PauseHandler {
	return &PauseHandler{
		c: c,
	}
}
...
func (h *PauseHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating PauseInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating PauseInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.PauseOutput
	if isTest {
...
func (h *PauseHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
