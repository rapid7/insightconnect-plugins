...
import (
	...
	"github.com/rapid7/komand-plugins/qualys_pc_scan/actions"
	"github.com/rapid7/komand-plugins/qualys_pc_scan/connection"
)

// CancelHandler is a handler
type CancelHandler struct {
	c *connection.Cache
}

// NewCancelHandler returns a new CancelHandler
func NewCancelHandler(c *connection.Cache) *CancelHandler {
...
func (h *CancelHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating CancelInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating CancelInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.CancelOutput
	if isTest {
...
func (h *CancelHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
