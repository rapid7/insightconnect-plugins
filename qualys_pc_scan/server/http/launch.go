...
// LaunchHandler is a handler
type LaunchHandler struct {
	c *connection.Cache
}

// NewLaunchHandler returns a new LaunchHandler
func NewLaunchHandler(c *connection.Cache) *LaunchHandler {
...
func (h *LaunchHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LaunchInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LaunchInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.LaunchOutput
	if isTest {
...
func (h *LaunchHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
