...
func NewListHandler(c *connection.Cache) *ListHandler {
	return &ListHandler{
		c: c,
	}
}
...
func (h *ListHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ListInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ListInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
...
func (h *ListHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
