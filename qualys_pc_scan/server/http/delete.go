...
// DeleteHandler is a handler
type DeleteHandler struct {
	c *connection.Cache
}

// NewDeleteHandler returns a new DeleteHandler
func NewDeleteHandler(c *connection.Cache) *DeleteHandler {
...
func (h *DeleteHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating DeleteInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating DeleteInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.DeleteOutput
	if isTest {
...
func (h *DeleteHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
