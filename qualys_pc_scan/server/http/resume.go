...
// ResumeHandler is a handler
type ResumeHandler struct {
	c *connection.Cache
}

// NewResumeHandler returns a new ResumeHandler
func NewResumeHandler(c *connection.Cache) *ResumeHandler {
...
func (h *ResumeHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ResumeInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ResumeInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.ResumeOutput
	if isTest {
...
func (h *ResumeHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
