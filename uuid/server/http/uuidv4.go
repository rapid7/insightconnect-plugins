...
import (
	...
	"github.com/rapid7/komand-plugins/uuid/actions"
	"github.com/rapid7/komand-plugins/uuid/connection"
)

// Uuidv4Handler is a handler
type Uuidv4Handler struct {
	c *connection.Cache
}

// NewUuidv4Handler returns a new Uuidv4Handler
func NewUuidv4Handler(c *connection.Cache) *Uuidv4Handler {
	return &Uuidv4Handler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *Uuidv4Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating Uuidv4Input: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating Uuidv4Input. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.Uuidv4Action{}
	var output *actions.Uuidv4Output
	if isTest {
...
func (h *Uuidv4Handler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
