...
import (
	...
	"github.com/rapid7/komand-plugins/uuid/server/http"
...
func main() {
	...
	case httpServer.FullCommand():
		...
		fmt.Printf("Plugin uuid booting HTTP Daemon on port %d...", *port)
...
func runOneOffAction(mode string, b *message.BodyV1, isDebug bool) error {
	...
	case "uuidv4":
		...
		for _, err = range input.Validate(l) {
			log.Println(fmt.Sprintf("Error while validating Uuidv4TriggerInput: %s", err.Error()))
		}
		if err != nil {
			return fmt.Errorf("Error while validating Uuidv4TriggerInput. Check logs for details")
		}
		a := &actions.Uuidv4Action{}
		runMethod := a.Run
		if mode == "test" {
			runMethod = a.Test
		}
...
func printInfo() {
	...
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "uuidv4", cli.Reset, "A V4 UUID", cli.Reset)
...
func printSampleInput(action string) error {
	...
	case "uuidv4":
		m.Type = "action_start"
		actionBytes, err := json.Marshal(actions.Uuidv4Input{})
		if err != nil {
			return err
		}
		b.Input = actionBytes
