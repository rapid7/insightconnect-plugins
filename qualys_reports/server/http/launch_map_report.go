...
// LaunchMapReportHandler is a handler
type LaunchMapReportHandler struct {
	c *connection.Cache
}

// NewLaunchMapReportHandler returns a new LaunchMapReportHandler
func NewLaunchMapReportHandler(c *connection.Cache) *LaunchMapReportHandler {
	return &LaunchMapReportHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *LaunchMapReportHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LaunchMapReportInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LaunchMapReportInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.LaunchMapReportAction{}
	var output *actions.LaunchMapReportOutput
	if isTest {
...
func (h *LaunchMapReportHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
