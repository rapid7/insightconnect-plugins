...
// LaunchComplianceReportHandler is a handler
type LaunchComplianceReportHandler struct {
	c *connection.Cache
}

// NewLaunchComplianceReportHandler returns a new LaunchComplianceReportHandler
func NewLaunchComplianceReportHandler(c *connection.Cache) *LaunchComplianceReportHandler {
	return &LaunchComplianceReportHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *LaunchComplianceReportHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LaunchComplianceReportInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LaunchComplianceReportInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.LaunchComplianceReportAction{}
	var output *actions.LaunchComplianceReportOutput
	if isTest {
...
func (h *LaunchComplianceReportHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
