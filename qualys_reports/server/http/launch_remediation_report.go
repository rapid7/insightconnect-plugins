...
// LaunchRemediationReportHandler is a handler
type LaunchRemediationReportHandler struct {
	c *connection.Cache
}

// NewLaunchRemediationReportHandler returns a new LaunchRemediationReportHandler
func NewLaunchRemediationReportHandler(c *connection.Cache) *LaunchRemediationReportHandler {
	return &LaunchRemediationReportHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *LaunchRemediationReportHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LaunchRemediationReportInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LaunchRemediationReportInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.LaunchRemediationReportAction{}
	var output *actions.LaunchRemediationReportOutput
	if isTest {
...
func (h *LaunchRemediationReportHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
