...
// LaunchPatchReportHandler is a handler
type LaunchPatchReportHandler struct {
	c *connection.Cache
}

// NewLaunchPatchReportHandler returns a new LaunchPatchReportHandler
func NewLaunchPatchReportHandler(c *connection.Cache) *LaunchPatchReportHandler {
	return &LaunchPatchReportHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *LaunchPatchReportHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LaunchPatchReportInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LaunchPatchReportInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.LaunchPatchReportAction{}
	var output *actions.LaunchPatchReportOutput
	if isTest {
...
func (h *LaunchPatchReportHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
