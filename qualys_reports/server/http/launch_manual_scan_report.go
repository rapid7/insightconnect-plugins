...
// LaunchManualScanReportHandler is a handler
type LaunchManualScanReportHandler struct {
	c *connection.Cache
}

// NewLaunchManualScanReportHandler returns a new LaunchManualScanReportHandler
func NewLaunchManualScanReportHandler(c *connection.Cache) *LaunchManualScanReportHandler {
	return &LaunchManualScanReportHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *LaunchManualScanReportHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LaunchManualScanReportInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LaunchManualScanReportInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.LaunchManualScanReportAction{}
	var output *actions.LaunchManualScanReportOutput
	if isTest {
...
func (h *LaunchManualScanReportHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
