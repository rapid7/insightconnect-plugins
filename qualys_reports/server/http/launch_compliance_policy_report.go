...
// LaunchCompliancePolicyReportHandler is a handler
type LaunchCompliancePolicyReportHandler struct {
	c *connection.Cache
}

// NewLaunchCompliancePolicyReportHandler returns a new LaunchCompliancePolicyReportHandler
func NewLaunchCompliancePolicyReportHandler(c *connection.Cache) *LaunchCompliancePolicyReportHandler {
	return &LaunchCompliancePolicyReportHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *LaunchCompliancePolicyReportHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating LaunchCompliancePolicyReportInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating LaunchCompliancePolicyReportInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.LaunchCompliancePolicyReportAction{}
	var output *actions.LaunchCompliancePolicyReportOutput
	if isTest {
...
func (h *LaunchCompliancePolicyReportHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
