...
// FetchHandler is a handler
type FetchHandler struct {
	c *connection.Cache
}

// NewFetchHandler returns a new FetchHandler
func NewFetchHandler(c *connection.Cache) *FetchHandler {
	return &FetchHandler{
		c: c,
	}
}
...
func (h *FetchHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating FetchInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating FetchInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.FetchOutput
	if isTest {
...
func (h *FetchHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
