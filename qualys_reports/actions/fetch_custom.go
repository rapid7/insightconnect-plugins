...
func (i *FetchInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *FetchOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *FetchAction) Run(conn *connection.Connection, input *FetchInput, log plog.Logger) (*FetchOutput, error) {
	reports := conn.CustomParams.Qualys.Reports
	file, _, err := reports.Fetch(context.Background(), qualys.ReportFetchOptions{
		ID: input.ID,
	})
	if err != nil {
...
func (a *FetchAction) Test(conn *connection.Connection, input *FetchInput, log plog.Logger) (*FetchOutput, error) {
	output := &FetchOutput{}
	// Your code here
