...
func (i *LaunchMapReportInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *LaunchMapReportOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *LaunchMapReportAction) Run(conn *connection.Connection, input *LaunchMapReportInput, log plog.Logger) (*LaunchMapReportOutput, error) {
	...
	report, _, err := reports.LaunchMapReport(context.Background(), qualys.MapReportLaunchOptions{
	...
	if err != nil {
		return &LaunchMapReportOutput{}, err
	}
	return &LaunchMapReportOutput{
		ID: report.ID,
	}, nil
}
...
func (a *LaunchMapReportAction) Test(conn *connection.Connection, input *LaunchMapReportInput, log plog.Logger) (*LaunchMapReportOutput, error) {
	output := &LaunchMapReportOutput{}
	// Your code here
