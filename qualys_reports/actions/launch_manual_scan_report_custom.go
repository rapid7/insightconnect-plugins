...
func (i *LaunchManualScanReportInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *LaunchManualScanReportOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *LaunchManualScanReportAction) Run(conn *connection.Connection, input *LaunchManualScanReportInput, log plog.Logger) (*LaunchManualScanReportOutput, error) {
	...
	report, _, err := reports.LaunchScanReport(context.Background(), qualys.ScanReportLaunchOptions{
		ReportLaunchOptions: rlo,
		IPRestriction:       input.IPRestriction,
		ReportRefs:          input.ReportRefs,
	})
	if err != nil {
		return &LaunchManualScanReportOutput{}, err
	}
	return &LaunchManualScanReportOutput{
		ID: report.ID,
	}, nil
}
...
func (a *LaunchManualScanReportAction) Test(conn *connection.Connection, input *LaunchManualScanReportInput, log plog.Logger) (*LaunchManualScanReportOutput, error) {
	output := &LaunchManualScanReportOutput{}
	// Your code here
