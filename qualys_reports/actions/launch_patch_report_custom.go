...
func (i *LaunchPatchReportInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *LaunchPatchReportOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *LaunchPatchReportAction) Run(conn *connection.Connection, input *LaunchPatchReportInput, log plog.Logger) (*LaunchPatchReportOutput, error) {
	...
	report, _, err := reports.LaunchPatchReport(context.Background(), qualys.PatchReportLaunchOptions{
		ReportLaunchOptions: rlo,
		IPs:                 input.Ips,
		AssetGroupIDs:       input.AssetGroupIds,
	})
	if err != nil {
		return &LaunchPatchReportOutput{}, err
	}
	return &LaunchPatchReportOutput{
		ID: report.ID,
	}, nil
}
...
func (a *LaunchPatchReportAction) Test(conn *connection.Connection, input *LaunchPatchReportInput, log plog.Logger) (*LaunchPatchReportOutput, error) {
	output := &LaunchPatchReportOutput{}
	// Your code here
