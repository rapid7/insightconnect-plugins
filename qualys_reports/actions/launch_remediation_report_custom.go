...
func (i *LaunchRemediationReportInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *LaunchRemediationReportOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *LaunchRemediationReportAction) Run(conn *connection.Connection, input *LaunchRemediationReportInput, log plog.Logger) (*LaunchRemediationReportOutput, error) {
	...
	report, _, err := reports.LaunchRemediationReport(context.Background(), qualys.RemediationReportLaunchOptions{
	...
	if err != nil {
		return &LaunchRemediationReportOutput{}, err
	}
	return &LaunchRemediationReportOutput{
		ID: report.ID,
	}, nil
}
...
func (a *LaunchRemediationReportAction) Test(conn *connection.Connection, input *LaunchRemediationReportInput, log plog.Logger) (*LaunchRemediationReportOutput, error) {
	output := &LaunchRemediationReportOutput{}
	// Your code here
