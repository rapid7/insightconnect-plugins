...
class Hash(insightconnect_plugin_runtime.Action):
    def __init__(self):
        super(self.__class__, self).__init__(
            name="hash", description=Component.DESCRIPTION, input=HashInput(), output=HashOutput()
        )
    ...
    def run(self, params={}):
        if self._is_not_valid_hash(params.get(Input.HASH)):
            raise PluginException(
                cause="An invalid or unsupported hash type was provided.", assistance="Please enter a MD5 or SHA1 hash."
            )
