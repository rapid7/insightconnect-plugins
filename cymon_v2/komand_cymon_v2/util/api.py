...
class CymonV2:
    ...
    def _call_api(self, method, url, multiple_results_field=None, params=None, json=None):
        if params is None:
            params = {}

        api_url = self.base_url + url

        headers = {"Authorization": "Bearer {}".format(self.token)} if self.token else None
        ...
        try:
            if multiple_results_field:
                ...
                while True:
                    response = request(method, api_url, params=params, headers=headers)
                ...
                response = request(method, api_url, params=params, json=json, headers=headers)
                response.raise_for_status()
                return komand.helper.clean(response.json())
        except Exception as e:
            raise Exception("Failed to get a response from API endpoint {}: {}".format(api_url, e))
