...
// IntegerToStringHandler is a handler
type IntegerToStringHandler struct {
	c *connection.Cache
}

// NewIntegerToStringHandler returns a new IntegerToStringHandler
func NewIntegerToStringHandler(c *connection.Cache) *IntegerToStringHandler {
	return &IntegerToStringHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *IntegerToStringHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating IntegerToStringInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating IntegerToStringInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.IntegerToStringAction{}
	var output *actions.IntegerToStringOutput
	if isTest {
...
func (h *IntegerToStringHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
