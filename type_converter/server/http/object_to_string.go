...
// ObjectToStringHandler is a handler
type ObjectToStringHandler struct {
	c *connection.Cache
}

// NewObjectToStringHandler returns a new ObjectToStringHandler
func NewObjectToStringHandler(c *connection.Cache) *ObjectToStringHandler {
...
func (h *ObjectToStringHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ObjectToStringInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ObjectToStringInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ObjectToStringAction{}
	var output *actions.ObjectToStringOutput
	if isTest {
...
func (h *ObjectToStringHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
