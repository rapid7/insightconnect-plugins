...
// StringToListHandler is a handler
type StringToListHandler struct {
	c *connection.Cache
}

// NewStringToListHandler returns a new StringToListHandler
func NewStringToListHandler(c *connection.Cache) *StringToListHandler {
	return &StringToListHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *StringToListHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating StringToListInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating StringToListInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.StringToListAction{}
	var output *actions.StringToListOutput
	if isTest {
...
func (h *StringToListHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
