...
// StringToBooleanHandler is a handler
type StringToBooleanHandler struct {
	c *connection.Cache
}

// NewStringToBooleanHandler returns a new StringToBooleanHandler
func NewStringToBooleanHandler(c *connection.Cache) *StringToBooleanHandler {
	return &StringToBooleanHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *StringToBooleanHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating StringToBooleanInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating StringToBooleanInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.StringToBooleanAction{}
	var output *actions.StringToBooleanOutput
	if isTest {
...
func (h *StringToBooleanHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
