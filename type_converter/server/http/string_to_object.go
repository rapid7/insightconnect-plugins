...
// StringToObjectHandler is a handler
type StringToObjectHandler struct {
	c *connection.Cache
}

// NewStringToObjectHandler returns a new StringToObjectHandler
func NewStringToObjectHandler(c *connection.Cache) *StringToObjectHandler {
	return &StringToObjectHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *StringToObjectHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating StringToObjectInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating StringToObjectInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.StringToObjectAction{}
	var output *actions.StringToObjectOutput
	if isTest {
...
func (h *StringToObjectHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
