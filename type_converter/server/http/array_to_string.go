...
// ArrayToStringHandler is a handler
type ArrayToStringHandler struct {
	c *connection.Cache
}

// NewArrayToStringHandler returns a new ArrayToStringHandler
func NewArrayToStringHandler(c *connection.Cache) *ArrayToStringHandler {
...
func (h *ArrayToStringHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ArrayToStringInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ArrayToStringInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ArrayToStringAction{}
	var output *actions.ArrayToStringOutput
	if isTest {
...
func (h *ArrayToStringHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
