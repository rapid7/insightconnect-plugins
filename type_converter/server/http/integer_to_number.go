...
// IntegerToNumberHandler is a handler
type IntegerToNumberHandler struct {
	c *connection.Cache
}

// NewIntegerToNumberHandler returns a new IntegerToNumberHandler
func NewIntegerToNumberHandler(c *connection.Cache) *IntegerToNumberHandler {
	return &IntegerToNumberHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *IntegerToNumberHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating IntegerToNumberInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating IntegerToNumberInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.IntegerToNumberAction{}
	var output *actions.IntegerToNumberOutput
	if isTest {
...
func (h *IntegerToNumberHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
