...
// NumberToIntegerHandler is a handler
type NumberToIntegerHandler struct {
	c *connection.Cache
}

// NewNumberToIntegerHandler returns a new NumberToIntegerHandler
func NewNumberToIntegerHandler(c *connection.Cache) *NumberToIntegerHandler {
	return &NumberToIntegerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *NumberToIntegerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating NumberToIntegerInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating NumberToIntegerInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.NumberToIntegerAction{}
	var output *actions.NumberToIntegerOutput
	if isTest {
...
func (h *NumberToIntegerHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
