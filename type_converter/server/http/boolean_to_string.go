...
// BooleanToStringHandler is a handler
type BooleanToStringHandler struct {
	c *connection.Cache
}

// NewBooleanToStringHandler returns a new BooleanToStringHandler
func NewBooleanToStringHandler(c *connection.Cache) *BooleanToStringHandler {
	return &BooleanToStringHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *BooleanToStringHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating BooleanToStringInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating BooleanToStringInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.BooleanToStringAction{}
	var output *actions.BooleanToStringOutput
	if isTest {
...
func (h *BooleanToStringHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
