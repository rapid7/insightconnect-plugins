...
// StringToFloatHandler is a handler
type StringToFloatHandler struct {
	c *connection.Cache
}

// NewStringToFloatHandler returns a new StringToFloatHandler
func NewStringToFloatHandler(c *connection.Cache) *StringToFloatHandler {
	return &StringToFloatHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *StringToFloatHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating StringToFloatInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating StringToFloatInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.StringToFloatAction{}
	var output *actions.StringToFloatOutput
	if isTest {
...
func (h *StringToFloatHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
