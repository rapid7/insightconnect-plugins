...
// IntegerToBooleanHandler is a handler
type IntegerToBooleanHandler struct {
	c *connection.Cache
}

// NewIntegerToBooleanHandler returns a new IntegerToBooleanHandler
func NewIntegerToBooleanHandler(c *connection.Cache) *IntegerToBooleanHandler {
	return &IntegerToBooleanHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *IntegerToBooleanHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating IntegerToBooleanInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating IntegerToBooleanInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.IntegerToBooleanAction{}
	var output *actions.IntegerToBooleanOutput
	if isTest {
...
func (h *IntegerToBooleanHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
