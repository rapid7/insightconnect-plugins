...
// StringToIntegerHandler is a handler
type StringToIntegerHandler struct {
	c *connection.Cache
}

// NewStringToIntegerHandler returns a new StringToIntegerHandler
func NewStringToIntegerHandler(c *connection.Cache) *StringToIntegerHandler {
	return &StringToIntegerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *StringToIntegerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating StringToIntegerInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating StringToIntegerInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.StringToIntegerAction{}
	var output *actions.StringToIntegerOutput
	if isTest {
...
func (h *StringToIntegerHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
