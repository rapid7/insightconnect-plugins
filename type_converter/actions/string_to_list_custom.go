...
func (i *StringToListInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *StringToListOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *StringToListAction) Run(conn *connection.Connection, input *StringToListInput, log plog.Logger) (*StringToListOutput, error) {
	output := &StringToListOutput{}

	if input.Delimiter == "" {
		fmt.Errorf("Delimiter not provided by user. Defaulting to the newline character")
		input.Delimiter = "\n"
	}

	output.Output = strings.Split(input.Input, input.Delimiter)
...
func (a *StringToListAction) Test(conn *connection.Connection, input *StringToListInput, log plog.Logger) (*StringToListOutput, error) {
