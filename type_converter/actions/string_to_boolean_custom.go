...
func (i *StringToBooleanInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *StringToBooleanOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *StringToBooleanAction) Run(conn *connection.Connection, input *StringToBooleanInput, log plog.Logger) (*StringToBooleanOutput, error) {
	output := &StringToBooleanOutput{}
	if strings.EqualFold(input.Input, "true") {
		output.Output = true
	} else {
		output.Output = false
	}
...
func (a *StringToBooleanAction) Test(conn *connection.Connection, input *StringToBooleanInput, log plog.Logger) (*StringToBooleanOutput, error) {
