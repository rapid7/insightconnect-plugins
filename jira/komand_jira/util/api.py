...
class JiraApi:
    ...
    def delete_user(self, accountId):
        url = (
            self.jira_client._options["server"] + "/rest/api/latest/user/?accountId=%s" % accountId
        )
    ...
    def find_users(self, query, max_results=10):
        return self.jira_client._fetch_pages(
            User, None, "user/search", 0, max_results, {"query": query}
        )
