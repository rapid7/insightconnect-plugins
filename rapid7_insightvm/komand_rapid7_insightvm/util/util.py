...
def adhoc_sql_report(connection, logger, report_payload: dict):
    ...
    if report_id is None:
        raise PluginException(cause="Error: Failed to create report, report ID was not returned with create request",
    ...
    if report_instance_id is None:
        raise PluginException(cause="Error: Failed to generate report, report instance ID was not returned with "
                                    "generate request.",
    ...
    while report_status_response['status'] != 'complete':
        time.sleep(30)
        report_status_response = resource_helper.resource_request(endpoint=endpoint)
        logger.info(f"Report ID {report_id}, instance ID {report_instance_id} status: "
                    f"{report_status_response['status']}")

        if ('status' in report_status_response) and (report_status_response['status'] in ("aborted", "failed")):
            raise PluginException(cause=f"Error: Report failed to generated with status "
                                        f"{report_status_response['status']}.",
