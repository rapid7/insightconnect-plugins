...
class Hunting(komand.Action):
    ...
    def run(self, params={}):
        ...
        if flow_name is "Netstat":
            self.network(flow_name)
        if flow_name is "ListProcesses":
    ...
    def administrative(self, flow_name, params={}):
        ...
        if flow_name is "Interrogate":
            flow_args.lightweight = True
            self.hunter_args()
        if flow_name is "KeepAlive":
            duration = params.get('duration')
            flow_args.duration = duration
            self.hunter_args()
        if flow_name is "OnlineNotification":
    ...
    def browser(self, flow_name, params={}):
        ...
        if flow_name is "ChromeHistory":
            ...
            self.hunter_args()
        if flow_name is "FirefoxHistory":
            get_archive = params.get('get_archive')
            history_path = params.get('history_path')
            path_type = params.get('pathtype')
            username = params.get('username')
    ...
    def collectors(self, flow_name, params={}):
        flow_args = self.grr_api.types.CreateFlowArgs(flow_name)
        if flow_name is "ArtifactCollectorFlow":
        ...
        if flow_name is "DumpACPITable":
            ...
            self.hunter_args()
        if flow_name is "DumpFlashImage":
    ...
    def filesystem(self, flow_name, params={}):
        if flow_name is 'list_volume_shadow_copies':
            flow_args = self.grr_api.types.CreateFlowArgs(flow_name)
        if flow_name is 'FileFinder':
            flow_args = self.grr_api.types.CreateFlowArgs(flow_name)
    ...
    def output_plugins(self, hunt_runner_args, params={}):
        ...
        if plugin_name is "BigQueryOutput":
    ...
    def processes(self, flow_name, params={}):
        flow_args = self.grr_api.types.CreateFlowArgs(flow_name)
        if flow_name is 'list_processes':
    ...
    def registry(self, flow_name, params={}):
        if flow_name is 'CollectRunKeyBinaries':
            flow_args = self.grr_api.types.CreateFlowArgs(flow_name)
        if flow_name is 'RegistryFinder':
            flow_args = self.grr_api.types.CreateFlowArgs(flow_name)
            conditions = params.get('conditions')
