...
class RPMHelper(object):
    ...
    def add_repo(self, url):
        ...
        if proc["rcode"] == 0:
            ...
            raise Exception("AddRepo: Repofile was not downloaded from %s" % url)
    ...
    def add_key(self, url):
        ...
        if proc["rcode"] == 0:
            ...
            if proc["rcode"] != 0:
                raise Exception("AddKey: Key was not imported: %s" % proc["stderr"].decode("utf-8"))
            else:
    ...
    def _modify_repofile(self, path):
        ...
        if not repo_ids:
            raise Exception("ModifyRepofile: No repo IDs in repofile")
        if not lines:
            raise Exception("ModifyRepofile: Writing empty repo file at %s" % path)
    ...
    def checksig(self, path):
        ...
        if proc["rcode"] != 0:
            raise Exception("CheckSig: " + (proc["stdout"] + proc["stderr"]).decode("utf-8"))
    ...
    def package_info(self, path):
        ...
        if proc["rcode"] == 0 and proc2["rcode"] == 0:
            return {
                "package": proc["stdout"].decode("utf-8"),
                "files": proc2["stdout"].decode("utf-8"),
            }
        else:
