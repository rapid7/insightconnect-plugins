...
// ToASCIIHandler is a handler
type ToASCIIHandler struct {
	c *connection.Cache
}

// NewToASCIIHandler returns a new ToASCIIHandler
func NewToASCIIHandler(c *connection.Cache) *ToASCIIHandler {
	return &ToASCIIHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *ToASCIIHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ToASCIIInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ToASCIIInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ToASCIIAction{}
	var output *actions.ToASCIIOutput
	if isTest {
...
func (h *ToASCIIHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
