...
// ToUnicodeHandler is a handler
type ToUnicodeHandler struct {
	c *connection.Cache
}

// NewToUnicodeHandler returns a new ToUnicodeHandler
func NewToUnicodeHandler(c *connection.Cache) *ToUnicodeHandler {
	return &ToUnicodeHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *ToUnicodeHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ToUnicodeInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ToUnicodeInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ToUnicodeAction{}
	var output *actions.ToUnicodeOutput
	if isTest {
...
func (h *ToUnicodeHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
