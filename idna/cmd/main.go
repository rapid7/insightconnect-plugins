...
import (
	...
	"github.com/rapid7/komand-plugins/idna/actions"
	"github.com/rapid7/komand-plugins/idna/connection"
	"github.com/rapid7/komand-plugins/idna/server/http"
...
func main() {
	...
	case httpServer.FullCommand():
		...
		fmt.Printf("Plugin idna booting HTTP Daemon on port %d...", *port)
...
func runOneOffAction(mode string, b *message.BodyV1, isDebug bool) error {
	...
	switch b.Action {
	case "to_ascii":
		input := &actions.ToASCIIInput{}
		if err = json.Unmarshal(b.Input, input); err != nil {
			return err
		}
		for _, err = range input.Validate(l) {
			log.Println(fmt.Sprintf("Error while validating ToASCIITriggerInput: %s", err.Error()))
		}
		if err != nil {
			return fmt.Errorf("Error while validating ToASCIITriggerInput. Check logs for details")
		}
		a := &actions.ToASCIIAction{}
		runMethod := a.Run
		if mode == "test" {
			runMethod = a.Test
		}
	...
	case "to_unicode":
		input := &actions.ToUnicodeInput{}
		if err = json.Unmarshal(b.Input, input); err != nil {
			return err
		}
		for _, err = range input.Validate(l) {
			log.Println(fmt.Sprintf("Error while validating ToUnicodeTriggerInput: %s", err.Error()))
		}
		if err != nil {
			return fmt.Errorf("Error while validating ToUnicodeTriggerInput. Check logs for details")
		}
		a := &actions.ToUnicodeAction{}
		runMethod := a.Run
		if mode == "test" {
			runMethod = a.Test
		}
...
func printInfo() {
	...
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "to_ascii", cli.Reset, "To ASCII (punycode)", cli.Reset)
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "to_unicode", cli.Reset, "To Unicode", cli.Reset)
...
func printSampleInput(action string) error {
	...
	case "to_ascii":
		m.Type = "action_start"
		actionBytes, err := json.Marshal(actions.ToASCIIInput{})
		if err != nil {
			return err
		}
		b.Input = actionBytes
	case "to_unicode":
		m.Type = "action_start"
		actionBytes, err := json.Marshal(actions.ToUnicodeInput{})
		if err != nil {
			return err
		}
		b.Input = actionBytes
