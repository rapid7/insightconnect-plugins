...
func (i *Sha512ExtractorInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *Sha512ExtractorOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *Sha512ExtractorAction) Run(conn *connection.Connection, input *Sha512ExtractorInput, log plog.Logger) (*Sha512ExtractorOutput, error) {
	output := &Sha512ExtractorOutput{}
	r := regexp.MustCompile("[A-Fa-f0-9]{128}")
	if input.File != nil {
		output.Sha512 = r.FindAllString(string(input.File), -1)
	}
	if input.Str != "" {
		s := r.FindAllString(input.Str, -1)
		output.Sha512 = append(output.Sha512, s...)
	}
	output.Sha512 = uniqueStrings(output.Sha512)
...
func (a *Sha512ExtractorAction) Test(conn *connection.Connection, input *Sha512ExtractorInput, log plog.Logger) (*Sha512ExtractorOutput, error) {
	output := &Sha512ExtractorOutput{}
	// Your code here
