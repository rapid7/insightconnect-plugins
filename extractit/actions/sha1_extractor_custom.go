...
func (i *Sha1ExtractorInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *Sha1ExtractorOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *Sha1ExtractorAction) Run(conn *connection.Connection, input *Sha1ExtractorInput, log plog.Logger) (*Sha1ExtractorOutput, error) {
	output := &Sha1ExtractorOutput{}
	r := regexp.MustCompile("[A-Fa-f0-9]{40}")
	if input.File != nil {
		output.Sha1 = r.FindAllString(string(input.File), -1)
	}
	if input.Str != "" {
		s := r.FindAllString(input.Str, -1)
		output.Sha1 = append(output.Sha1, s...)
	}
	output.Sha1 = uniqueStrings(output.Sha1)
...
func (a *Sha1ExtractorAction) Test(conn *connection.Connection, input *Sha1ExtractorInput, log plog.Logger) (*Sha1ExtractorOutput, error) {
	output := &Sha1ExtractorOutput{}
	// Your code here
