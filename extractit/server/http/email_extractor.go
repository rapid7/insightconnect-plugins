...
// EmailExtractorHandler is a handler
type EmailExtractorHandler struct {
	c *connection.Cache
}

// NewEmailExtractorHandler returns a new EmailExtractorHandler
func NewEmailExtractorHandler(c *connection.Cache) *EmailExtractorHandler {
	return &EmailExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *EmailExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating EmailExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating EmailExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.EmailExtractorAction{}
	var output *actions.EmailExtractorOutput
	if isTest {
...
func (h *EmailExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
