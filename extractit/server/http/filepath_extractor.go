...
// FilepathExtractorHandler is a handler
type FilepathExtractorHandler struct {
	c *connection.Cache
}

// NewFilepathExtractorHandler returns a new FilepathExtractorHandler
func NewFilepathExtractorHandler(c *connection.Cache) *FilepathExtractorHandler {
	return &FilepathExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *FilepathExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating FilepathExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating FilepathExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.FilepathExtractorAction{}
	var output *actions.FilepathExtractorOutput
	if isTest {
...
func (h *FilepathExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
