...
// Sha256ExtractorHandler is a handler
type Sha256ExtractorHandler struct {
	c *connection.Cache
}

// NewSha256ExtractorHandler returns a new Sha256ExtractorHandler
func NewSha256ExtractorHandler(c *connection.Cache) *Sha256ExtractorHandler {
	return &Sha256ExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *Sha256ExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating Sha256ExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating Sha256ExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.Sha256ExtractorAction{}
	var output *actions.Sha256ExtractorOutput
	if isTest {
...
func (h *Sha256ExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
