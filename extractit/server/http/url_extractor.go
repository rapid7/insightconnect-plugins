...
// URLExtractorHandler is a handler
type URLExtractorHandler struct {
	c *connection.Cache
}

// NewURLExtractorHandler returns a new URLExtractorHandler
func NewURLExtractorHandler(c *connection.Cache) *URLExtractorHandler {
	return &URLExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *URLExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating URLExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating URLExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.URLExtractorAction{}
	var output *actions.URLExtractorOutput
	if isTest {
...
func (h *URLExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
