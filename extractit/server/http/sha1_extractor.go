...
// Sha1ExtractorHandler is a handler
type Sha1ExtractorHandler struct {
	c *connection.Cache
}

// NewSha1ExtractorHandler returns a new Sha1ExtractorHandler
func NewSha1ExtractorHandler(c *connection.Cache) *Sha1ExtractorHandler {
	return &Sha1ExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *Sha1ExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating Sha1ExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating Sha1ExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.Sha1ExtractorAction{}
	var output *actions.Sha1ExtractorOutput
	if isTest {
...
func (h *Sha1ExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
