...
// MacExtractorHandler is a handler
type MacExtractorHandler struct {
	c *connection.Cache
}

// NewMacExtractorHandler returns a new MacExtractorHandler
func NewMacExtractorHandler(c *connection.Cache) *MacExtractorHandler {
	return &MacExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *MacExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating MacExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating MacExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.MacExtractorAction{}
	var output *actions.MacExtractorOutput
	if isTest {
...
func (h *MacExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
