...
// Md5ExtractorHandler is a handler
type Md5ExtractorHandler struct {
	c *connection.Cache
}

// NewMd5ExtractorHandler returns a new Md5ExtractorHandler
func NewMd5ExtractorHandler(c *connection.Cache) *Md5ExtractorHandler {
	return &Md5ExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *Md5ExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating Md5ExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating Md5ExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.Md5ExtractorAction{}
	var output *actions.Md5ExtractorOutput
	if isTest {
...
func (h *Md5ExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
