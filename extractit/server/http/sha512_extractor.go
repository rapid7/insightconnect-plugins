...
// Sha512ExtractorHandler is a handler
type Sha512ExtractorHandler struct {
	c *connection.Cache
}

// NewSha512ExtractorHandler returns a new Sha512ExtractorHandler
func NewSha512ExtractorHandler(c *connection.Cache) *Sha512ExtractorHandler {
	return &Sha512ExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *Sha512ExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating Sha512ExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating Sha512ExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.Sha512ExtractorAction{}
	var output *actions.Sha512ExtractorOutput
	if isTest {
...
func (h *Sha512ExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
