...
// IPExtractorHandler is a handler
type IPExtractorHandler struct {
	c *connection.Cache
}

// NewIPExtractorHandler returns a new IPExtractorHandler
func NewIPExtractorHandler(c *connection.Cache) *IPExtractorHandler {
	return &IPExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *IPExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating IPExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating IPExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.IPExtractorAction{}
	var output *actions.IPExtractorOutput
	if isTest {
...
func (h *IPExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
