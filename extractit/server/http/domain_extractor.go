...
// DomainExtractorHandler is a handler
type DomainExtractorHandler struct {
	c *connection.Cache
}

// NewDomainExtractorHandler returns a new DomainExtractorHandler
func NewDomainExtractorHandler(c *connection.Cache) *DomainExtractorHandler {
	return &DomainExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *DomainExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating DomainExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating DomainExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.DomainExtractorAction{}
	var output *actions.DomainExtractorOutput
	if isTest {
...
func (h *DomainExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
