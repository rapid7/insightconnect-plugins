...
// DateExtractorHandler is a handler
type DateExtractorHandler struct {
	c *connection.Cache
}

// NewDateExtractorHandler returns a new DateExtractorHandler
func NewDateExtractorHandler(c *connection.Cache) *DateExtractorHandler {
	return &DateExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *DateExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating DateExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating DateExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.DateExtractorAction{}
	var output *actions.DateExtractorOutput
	if isTest {
...
func (h *DateExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
