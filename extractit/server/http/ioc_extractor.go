...
// IocExtractorHandler is a handler
type IocExtractorHandler struct {
	c *connection.Cache
}

// NewIocExtractorHandler returns a new IocExtractorHandler
func NewIocExtractorHandler(c *connection.Cache) *IocExtractorHandler {
	return &IocExtractorHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *IocExtractorHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating IocExtractorInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating IocExtractorInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.IocExtractorAction{}
	var output *actions.IocExtractorOutput
	if isTest {
...
func (h *IocExtractorHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
