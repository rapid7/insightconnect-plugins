...
class Connection(komand.Connection):
    ...
    def agents_summary(self, site_ids, account_ids):
        return self._call_api(
            "GET", "private/agents/summary", None, {"siteIds": site_ids, "accountIds": account_ids}
        )
    ...
    def download_file(self, agent_filter: dict, password: str):
        ...
        response = self._call_api(
            "GET", activities["data"][0]["data"]["filePath"][1:], full_response=True
        )
        ...
        return {
            ...
            "content": base64.b64encode(
                downloaded_zipfile.read(downloaded_zipfile.infolist()[-1])
            ).decode("utf-8"),
        }
    ...
    def threats_fetch_file(self, password: str, agents_filter: dict) -> int:
        self.get_auth_token(self.url, self.username, self.password)
        return self._call_api(
            "POST", "threats/fetch-file", {"data": {"password": password}, "filter": agents_filter}
        )

    def agents_support_action(self, action: str, agents_filter: str, module: str):
        return self._call_api(
            "POST",
            "private/agents/support-actions/{}".format(action),
            {"filter": agents_filter, "data": {"module": module}},
        )
    ...
    def blacklist_by_content_hash(self, hash_value: str):
        ...
        headers = self.make_token_header()
        body = {"filter": {"contentHash": hash_value}, "data": {"targetScope": "site"}}
    ...
    def blacklist_by_ioc_hash(self, hash_value: str, agent_id: str):
        ...
        body = {"data": [{"hash": hash_value, "agentId": agent_id}]}
    ...
    def create_ioc_threat(
        self, hash_, group_id, path, agent_id, annotation=None, annotation_url=None
    ):
        body = {
            "data": [
                {
    ...
    def mark_as_benign(self, threat_id, whitening_option, target_scope):
        ...
        return self._call_api("POST", "threats/mark-as-benign", body)["data"]["affected"]
    ...
    def mark_as_threat(self, threat_id, whitening_option, target_scope):
        ...
        return self._call_api("POST", "threats/mark-as-threat", body)["data"]["affected"]
    ...
    def get_item_ids_by_hash(self, blacklist_hash: str):
        response = self._call_api(
            "GET", "restrictions", params={"type": "black_hash", "value": blacklist_hash}
        )
