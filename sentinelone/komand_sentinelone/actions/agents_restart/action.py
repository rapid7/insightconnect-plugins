...
class AgentsRestart(komand.Action):
    ...
    def run(self, params={}):
        ...
        if (
            "ids" not in agent_filter
            and "groupIds" not in agent_filter
            and "filterId" not in agent_filter
        ):
            self.logger.error(
                "One of the following filter arguments must be supplied - ids, groupIds, filterId"
            )
        ...
        return {
            Output.AFFECTED: self.connection.agents_action("restart-machine", agent_filter).get(
                "affected", 0
            )
        }
