...
class AppendLineToFile(komand.Action):
    ...
    def run(self, params={}):
        ...
        try:
            ...
            commit_hash = git_repository.commit("Update file {}".format(file_path))
            result["commit_id"] = commit_hash
            git_repository.push()

            self.logger.info("Run: File {} updated successfully".format(file_path))
            result["commit_url"] = git_repository.get_commit_url(commit_hash)
            result["success"] = True
        except Exception as e:
            self.logger.error("AppendLineToFile: Exception: Failed to update {}:\n{}".format(file_path, str(e)))
