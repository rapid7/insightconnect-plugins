...
// ExtractHandler is a handler
type ExtractHandler struct {
	c *connection.Cache
}

// NewExtractHandler returns a new ExtractHandler
func NewExtractHandler(c *connection.Cache) *ExtractHandler {
...
func (h *ExtractHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ExtractInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ExtractInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.ExtractOutput
	if isTest {
...
func (h *ExtractHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
