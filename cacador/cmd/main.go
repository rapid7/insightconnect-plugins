...
import (
	...
	"github.com/rapid7/komand-plugins/cacador/actions"
	"github.com/rapid7/komand-plugins/cacador/connection"
	"github.com/rapid7/komand-plugins/cacador/server/http"
...
var (
	...
	Description = "Cacador IOC Extraction"
...
func main() {
	...
	case httpServer.FullCommand():
		...
		fmt.Printf("Plugin cacador booting HTTP Daemon on port %d...", *port)
...
func runOneOffAction(mode string, b *message.BodyV1, isDebug bool) error {
	...
	switch b.Action {
	case "extract":
		input := &actions.ExtractInput{}
		if err = json.Unmarshal(b.Input, input); err != nil {
			return err
		}
		for _, err = range input.Validate(l) {
			log.Println(fmt.Sprintf("Error while validating ExtractTriggerInput: %s", err.Error()))
		}
		if err != nil {
			return fmt.Errorf("Error while validating ExtractTriggerInput. Check logs for details")
		}
		a := &actions.ExtractAction{}
		runMethod := a.Run
		if mode == "test" {
			runMethod = a.Test
		}
...
func printInfo() {
	...
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "extract", cli.Reset, "Extract IOCS", cli.Reset)
...
func printSampleInput(action string) error {
	...
	case "extract":
		m.Type = "action_start"
		actionBytes, err := json.Marshal(actions.ExtractInput{})
		if err != nil {
			return err
		}
		b.Input = actionBytes
