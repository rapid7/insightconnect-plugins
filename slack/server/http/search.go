...
// SearchHandler is a handler
type SearchHandler struct {
	c *connection.Cache
}

// NewSearchHandler returns a new SearchHandler
func NewSearchHandler(c *connection.Cache) *SearchHandler {
...
func (h *SearchHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating SearchInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating SearchInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.SearchOutput
	if isTest {
...
func (h *SearchHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
