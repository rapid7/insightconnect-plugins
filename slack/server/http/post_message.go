...
// PostMessageHandler is a handler
type PostMessageHandler struct {
	c *connection.Cache
}

// NewPostMessageHandler returns a new PostMessageHandler
func NewPostMessageHandler(c *connection.Cache) *PostMessageHandler {
...
func (h *PostMessageHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating PostMessageInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating PostMessageInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.PostMessageAction{}
	var output *actions.PostMessageOutput
	if isTest {
...
func (h *PostMessageHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
