...
// MessageWithFileTriggerHandler is a handler
type MessageWithFileTriggerHandler struct {
	c *connection.Cache
}

// NewMessageWithFileTriggerHandler returns a new MessageWithFileHandler
func NewMessageWithFileTriggerHandler(c *connection.Cache) *MessageWithFileTriggerHandler {
	return &MessageWithFileTriggerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *MessageWithFileTriggerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating MessageWithFileInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating MessageWithFileInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	trigger := triggers.NewMessageWithFileTrigger(nil, response.Meta)
	var output *triggers.MessageWithFileTriggerOutput
...
func (h *MessageWithFileTriggerHandler) replyWithError(response *message.TriggerEvent, w http.ResponseWriter, err error) {
