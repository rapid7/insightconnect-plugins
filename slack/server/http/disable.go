...
// DisableHandler is a handler
type DisableHandler struct {
	c *connection.Cache
}

// NewDisableHandler returns a new DisableHandler
func NewDisableHandler(c *connection.Cache) *DisableHandler {
...
func (h *DisableHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating DisableInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating DisableInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.DisableOutput
	if isTest {
...
func (h *DisableHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
