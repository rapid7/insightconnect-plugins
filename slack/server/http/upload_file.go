...
// UploadFileHandler is a handler
type UploadFileHandler struct {
	c *connection.Cache
}

// NewUploadFileHandler returns a new UploadFileHandler
func NewUploadFileHandler(c *connection.Cache) *UploadFileHandler {
...
func (h *UploadFileHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating UploadFileInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating UploadFileInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.UploadFileOutput
	if isTest {
...
func (h *UploadFileHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
