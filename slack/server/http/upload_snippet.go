...
// UploadSnippetHandler is a handler
type UploadSnippetHandler struct {
	c *connection.Cache
}

// NewUploadSnippetHandler returns a new UploadSnippetHandler
func NewUploadSnippetHandler(c *connection.Cache) *UploadSnippetHandler {
	return &UploadSnippetHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *UploadSnippetHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating UploadSnippetInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating UploadSnippetInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.UploadSnippetAction{}
	var output *actions.UploadSnippetOutput
	if isTest {
...
func (h *UploadSnippetHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
