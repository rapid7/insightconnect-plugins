...
// EnableHandler is a handler
type EnableHandler struct {
	c *connection.Cache
}

// NewEnableHandler returns a new EnableHandler
func NewEnableHandler(c *connection.Cache) *EnableHandler {
...
func (h *EnableHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating EnableInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating EnableInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.EnableOutput
	if isTest {
...
func (h *EnableHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
