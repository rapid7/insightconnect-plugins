...
// EventTriggerHandler is a handler
type EventTriggerHandler struct {
	c *connection.Cache
}

// NewEventTriggerHandler returns a new EventHandler
func NewEventTriggerHandler(c *connection.Cache) *EventTriggerHandler {
...
func (h *EventTriggerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating EventInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating EventInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	trigger := triggers.NewEventTrigger(nil, response.Meta)
	var output *triggers.EventTriggerOutput
...
func (h *EventTriggerHandler) replyWithError(response *message.TriggerEvent, w http.ResponseWriter, err error) {
