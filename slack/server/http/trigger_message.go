...
// MessageTriggerHandler is a handler
type MessageTriggerHandler struct {
	c *connection.Cache
}

// NewMessageTriggerHandler returns a new MessageHandler
func NewMessageTriggerHandler(c *connection.Cache) *MessageTriggerHandler {
	return &MessageTriggerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *MessageTriggerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating MessageInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating MessageInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	trigger := triggers.NewMessageTrigger(nil, response.Meta)
	var output *triggers.MessageTriggerOutput
...
func (h *MessageTriggerHandler) replyWithError(response *message.TriggerEvent, w http.ResponseWriter, err error) {
