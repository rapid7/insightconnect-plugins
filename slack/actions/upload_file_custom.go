...
func (i *UploadFileInput) Validate(log plog.Logger) []error {
	i.channels = strings.Split(i.Channels, ",")
	err := ioutil.WriteFile("/tmp/upload", i.Content, 0644)
	if err != nil {
		return []error{err}
	}
	return nil
...
func (o *UploadFileOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *UploadFileAction) Run(conn *connection.Connection, input *UploadFileInput, log plog.Logger) (*UploadFileOutput, error) {
	...
	file, err := conn.API.UploadFile(params)
	if err != nil {
		return nil, fmt.Errorf("File upload failed: %s", err)

	}
	output := &UploadFileOutput{File: file}
...
func (a *UploadFileAction) Test(conn *connection.Connection, input *UploadFileInput, log plog.Logger) (*UploadFileOutput, error) {
