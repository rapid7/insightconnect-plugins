...
func (i *DisableInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *DisableOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *DisableAction) Run(conn *connection.Connection, input *DisableInput, log plog.Logger) (*DisableOutput, error) {
	output := &DisableOutput{}

	users, err := conn.API.GetUsers()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Disable: %v", err)
	}
	id := ""
	...
	if id == "" {
		fmt.Fprintf(os.Stderr, "Disable: ID is null")
	}
	...
	//enable the user for this email
	output.Success = true
	return output, nil
}
...
func (a *DisableAction) Test(conn *connection.Connection, input *DisableInput, log plog.Logger) (*DisableOutput, error) {
	output := &DisableOutput{}
	// Your code here
