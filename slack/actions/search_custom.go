...
func (i *SearchInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *SearchOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *SearchAction) Run(conn *connection.Connection, input *SearchInput, log plog.Logger) (*SearchOutput, error) {
	output := &SearchOutput{}

	params := slack.NewSearchParameters()
	params.Count = input.Count
	params.Sort = input.Sort
	params.Highlight = input.Highlight
	params.Page = input.Page
	params.SortDirection = input.SortDir

	// TODO: ignore any file matches for now.
	msgs, _, err := conn.API.Search(input.Query, params)

	if err != nil {
		return nil, err
	}

	output.Matches = msgs.Matches
	output.Total = msgs.Paging.Total
	output.Count = msgs.Paging.Count
	output.Page = msgs.Paging.Page
	output.Pages = msgs.Paging.Pages
...
func (a *SearchAction) Test(conn *connection.Connection, input *SearchInput, log plog.Logger) (*SearchOutput, error) {
	output := &SearchOutput{}
	// Your code here
