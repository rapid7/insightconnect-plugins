...
import (
	...
	"github.com/rapid7/komand-plugins/url_extractor/server/http"
...
func main() {
	...
	case httpServer.FullCommand():
		...
		fmt.Printf("Plugin url_extractor booting HTTP Daemon on port %d...", *port)
...
func runOneOffAction(mode string, b *message.BodyV1, isDebug bool) error {
	...
	case "parse_urls":
		...
		for _, err = range input.Validate(l) {
			log.Println(fmt.Sprintf("Error while validating ParseUrlsTriggerInput: %s", err.Error()))
		}
		if err != nil {
			return fmt.Errorf("Error while validating ParseUrlsTriggerInput. Check logs for details")
		}
		a := &actions.ParseUrlsAction{}
		runMethod := a.Run
		if mode == "test" {
			runMethod = a.Test
		}
...
func printInfo() {
	...
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "parse_urls", cli.Reset, "Parses URLs from text", cli.Reset)
...
func printSampleInput(action string) error {
	...
	case "parse_urls":
		m.Type = "action_start"
		actionBytes, err := json.Marshal(actions.ParseUrlsInput{})
		if err != nil {
			return err
		}
		b.Input = actionBytes
