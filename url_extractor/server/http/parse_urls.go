...
import (
	...
	"github.com/rapid7/komand-plugins/url_extractor/actions"
	"github.com/rapid7/komand-plugins/url_extractor/connection"
)

// ParseUrlsHandler is a handler
type ParseUrlsHandler struct {
	c *connection.Cache
}

// NewParseUrlsHandler returns a new ParseUrlsHandler
func NewParseUrlsHandler(c *connection.Cache) *ParseUrlsHandler {
	return &ParseUrlsHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *ParseUrlsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ParseUrlsInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ParseUrlsInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ParseUrlsAction{}
	var output *actions.ParseUrlsOutput
	if isTest {
...
func (h *ParseUrlsHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
