...
// TransformHandler is a handler
type TransformHandler struct {
	c *connection.Cache
}

// NewTransformHandler returns a new TransformHandler
func NewTransformHandler(c *connection.Cache) *TransformHandler {
...
func (h *TransformHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating TransformInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating TransformInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.TransformOutput
	if isTest {
...
func (h *TransformHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
