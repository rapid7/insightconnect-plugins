...
import (
	...
	"github.com/rapid7/komand-plugins/json/actions"
	"github.com/rapid7/komand-plugins/json/connection"
	"github.com/rapid7/komand-plugins/json/server/http"
...
var (
	...
	Description = "Transform JSON data per template"
...
func main() {
	...
	case httpServer.FullCommand():
		...
		fmt.Printf("Plugin json booting HTTP Daemon on port %d...", *port)
...
func runOneOffAction(mode string, b *message.BodyV1, isDebug bool) error {
	...
	switch b.Action {
	case "transform":
		input := &actions.TransformInput{}
		if err = json.Unmarshal(b.Input, input); err != nil {
			return err
		}
		for _, err = range input.Validate(l) {
			log.Println(fmt.Sprintf("Error while validating TransformTriggerInput: %s", err.Error()))
		}
		if err != nil {
			return fmt.Errorf("Error while validating TransformTriggerInput. Check logs for details")
		}
		a := &actions.TransformAction{}
		runMethod := a.Run
		if mode == "test" {
			runMethod = a.Test
		}
...
func printInfo() {
	...
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "transform", cli.Reset, "Transform JSON data per template", cli.Reset)
...
func printSampleInput(action string) error {
	...
	case "transform":
		m.Type = "action_start"
		actionBytes, err := json.Marshal(actions.TransformInput{})
		if err != nil {
			return err
		}
		b.Input = actionBytes
