...
import (
	...
	"github.com/rapid7/komand-plugins/json/actions/implementation"
...
func (i *TransformInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *TransformOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *TransformAction) Run(conn *connection.Connection, input *TransformInput, log plog.Logger) (*TransformOutput, error) {
	output := &TransformOutput{}
	sourceData := input.SourceData
	extractionTemplate := input.ExtractionTemplate
	targetTemplate := input.TargetTemplate

	targetData := implementation.Transform(sourceData, extractionTemplate, targetTemplate)

	output.TargetData = targetData.(map[string]interface{})
...
func (a *TransformAction) Test(conn *connection.Connection, input *TransformInput, log plog.Logger) (*TransformOutput, error) {
	output := &TransformOutput{}
	// Your code here
