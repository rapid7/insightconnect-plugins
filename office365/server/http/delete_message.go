...
// DeleteMessageHandler is a handler
type DeleteMessageHandler struct {
	c *connection.Cache
}

// NewDeleteMessageHandler returns a new DeleteMessageHandler
func NewDeleteMessageHandler(c *connection.Cache) *DeleteMessageHandler {
...
func (h *DeleteMessageHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating DeleteMessageInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating DeleteMessageInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.DeleteMessageAction{}
	var output *actions.DeleteMessageOutput
	if isTest {
...
func (h *DeleteMessageHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
