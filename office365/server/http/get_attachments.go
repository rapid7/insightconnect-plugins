...
// GetAttachmentsHandler is a handler
type GetAttachmentsHandler struct {
	c *connection.Cache
}

// NewGetAttachmentsHandler returns a new GetAttachmentsHandler
func NewGetAttachmentsHandler(c *connection.Cache) *GetAttachmentsHandler {
	return &GetAttachmentsHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *GetAttachmentsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating GetAttachmentsInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating GetAttachmentsInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.GetAttachmentsAction{}
	var output *actions.GetAttachmentsOutput
	if isTest {
...
func (h *GetAttachmentsHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
