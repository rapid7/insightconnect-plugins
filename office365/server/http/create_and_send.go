...
// CreateAndSendHandler is a handler
type CreateAndSendHandler struct {
	c *connection.Cache
}

// NewCreateAndSendHandler returns a new CreateAndSendHandler
func NewCreateAndSendHandler(c *connection.Cache) *CreateAndSendHandler {
	return &CreateAndSendHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *CreateAndSendHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating CreateAndSendInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating CreateAndSendInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.CreateAndSendAction{}
	var output *actions.CreateAndSendOutput
	if isTest {
...
func (h *CreateAndSendHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
