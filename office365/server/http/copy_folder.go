...
// CopyFolderHandler is a handler
type CopyFolderHandler struct {
	c *connection.Cache
}

// NewCopyFolderHandler returns a new CopyFolderHandler
func NewCopyFolderHandler(c *connection.Cache) *CopyFolderHandler {
...
func (h *CopyFolderHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating CopyFolderInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating CopyFolderInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.CopyFolderAction{}
	var output *actions.CopyFolderOutput
	if isTest {
...
func (h *CopyFolderHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
