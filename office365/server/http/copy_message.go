...
// CopyMessageHandler is a handler
type CopyMessageHandler struct {
	c *connection.Cache
}

// NewCopyMessageHandler returns a new CopyMessageHandler
func NewCopyMessageHandler(c *connection.Cache) *CopyMessageHandler {
	return &CopyMessageHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *CopyMessageHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating CopyMessageInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating CopyMessageInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.CopyMessageAction{}
	var output *actions.CopyMessageOutput
	if isTest {
...
func (h *CopyMessageHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
