...
// GetFoldersHandler is a handler
type GetFoldersHandler struct {
	c *connection.Cache
}

// NewGetFoldersHandler returns a new GetFoldersHandler
func NewGetFoldersHandler(c *connection.Cache) *GetFoldersHandler {
	return &GetFoldersHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *GetFoldersHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating GetFoldersInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating GetFoldersInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.GetFoldersAction{}
	var output *actions.GetFoldersOutput
	if isTest {
...
func (h *GetFoldersHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
