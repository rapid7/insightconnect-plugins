...
// CreateAttachmentHandler is a handler
type CreateAttachmentHandler struct {
	c *connection.Cache
}

// NewCreateAttachmentHandler returns a new CreateAttachmentHandler
func NewCreateAttachmentHandler(c *connection.Cache) *CreateAttachmentHandler {
...
func (h *CreateAttachmentHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating CreateAttachmentInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating CreateAttachmentInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.CreateAttachmentAction{}
	var output *actions.CreateAttachmentOutput
	if isTest {
...
func (h *CreateAttachmentHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
