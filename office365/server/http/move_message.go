...
// MoveMessageHandler is a handler
type MoveMessageHandler struct {
	c *connection.Cache
}

// NewMoveMessageHandler returns a new MoveMessageHandler
func NewMoveMessageHandler(c *connection.Cache) *MoveMessageHandler {
	return &MoveMessageHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *MoveMessageHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating MoveMessageInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating MoveMessageInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.MoveMessageAction{}
	var output *actions.MoveMessageOutput
	if isTest {
...
func (h *MoveMessageHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
