...
// ForwardMessageHandler is a handler
type ForwardMessageHandler struct {
	c *connection.Cache
}

// NewForwardMessageHandler returns a new ForwardMessageHandler
func NewForwardMessageHandler(c *connection.Cache) *ForwardMessageHandler {
...
func (h *ForwardMessageHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ForwardMessageInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ForwardMessageInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ForwardMessageAction{}
	var output *actions.ForwardMessageOutput
	if isTest {
...
func (h *ForwardMessageHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
