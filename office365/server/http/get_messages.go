...
// GetMessagesHandler is a handler
type GetMessagesHandler struct {
	c *connection.Cache
}

// NewGetMessagesHandler returns a new GetMessagesHandler
func NewGetMessagesHandler(c *connection.Cache) *GetMessagesHandler {
...
func (h *GetMessagesHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating GetMessagesInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating GetMessagesInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.GetMessagesAction{}
	var output *actions.GetMessagesOutput
	if isTest {
...
func (h *GetMessagesHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
