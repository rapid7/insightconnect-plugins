...
// GetContactsHandler is a handler
type GetContactsHandler struct {
	c *connection.Cache
}

// NewGetContactsHandler returns a new GetContactsHandler
func NewGetContactsHandler(c *connection.Cache) *GetContactsHandler {
...
func (h *GetContactsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating GetContactsInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating GetContactsInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.GetContactsAction{}
	var output *actions.GetContactsOutput
	if isTest {
...
func (h *GetContactsHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
