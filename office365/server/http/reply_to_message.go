...
// ReplyToMessageHandler is a handler
type ReplyToMessageHandler struct {
	c *connection.Cache
}

// NewReplyToMessageHandler returns a new ReplyToMessageHandler
func NewReplyToMessageHandler(c *connection.Cache) *ReplyToMessageHandler {
	return &ReplyToMessageHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *ReplyToMessageHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ReplyToMessageInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ReplyToMessageInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ReplyToMessageAction{}
	var output *actions.ReplyToMessageOutput
	if isTest {
...
func (h *ReplyToMessageHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
