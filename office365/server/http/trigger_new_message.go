...
// NewMessageTriggerHandler is a handler
type NewMessageTriggerHandler struct {
	c *connection.Cache
}

// NewNewMessageTriggerHandler returns a new NewMessageHandler
func NewNewMessageTriggerHandler(c *connection.Cache) *NewMessageTriggerHandler {
	return &NewMessageTriggerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *NewMessageTriggerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating NewMessageInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating NewMessageInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	trigger := triggers.NewNewMessageTrigger(nil, response.Meta)
	var output *triggers.NewMessageTriggerOutput
...
func (h *NewMessageTriggerHandler) replyWithError(response *message.TriggerEvent, w http.ResponseWriter, err error) {
