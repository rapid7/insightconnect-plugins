...
// DeleteAttachmentHandler is a handler
type DeleteAttachmentHandler struct {
	c *connection.Cache
}

// NewDeleteAttachmentHandler returns a new DeleteAttachmentHandler
func NewDeleteAttachmentHandler(c *connection.Cache) *DeleteAttachmentHandler {
...
func (h *DeleteAttachmentHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating DeleteAttachmentInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating DeleteAttachmentInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.DeleteAttachmentAction{}
	var output *actions.DeleteAttachmentOutput
	if isTest {
...
func (h *DeleteAttachmentHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
