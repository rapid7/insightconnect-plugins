...
// MoveFolderHandler is a handler
type MoveFolderHandler struct {
	c *connection.Cache
}

// NewMoveFolderHandler returns a new MoveFolderHandler
func NewMoveFolderHandler(c *connection.Cache) *MoveFolderHandler {
	return &MoveFolderHandler{
		c: c,
	}
}
...
func (h *MoveFolderHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating MoveFolderInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating MoveFolderInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.MoveFolderAction{}
	var output *actions.MoveFolderOutput
	if isTest {
...
func (h *MoveFolderHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
