...
// DeleteFolderHandler is a handler
type DeleteFolderHandler struct {
	c *connection.Cache
}

// NewDeleteFolderHandler returns a new DeleteFolderHandler
func NewDeleteFolderHandler(c *connection.Cache) *DeleteFolderHandler {
...
func (h *DeleteFolderHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating DeleteFolderInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating DeleteFolderInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.DeleteFolderAction{}
	var output *actions.DeleteFolderOutput
	if isTest {
...
func (h *DeleteFolderHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
