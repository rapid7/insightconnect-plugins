...
// MostRecentlySentHandler is a handler
type MostRecentlySentHandler struct {
	c *connection.Cache
}

// NewMostRecentlySentHandler returns a new MostRecentlySentHandler
func NewMostRecentlySentHandler(c *connection.Cache) *MostRecentlySentHandler {
	return &MostRecentlySentHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *MostRecentlySentHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating MostRecentlySentInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating MostRecentlySentInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.MostRecentlySentAction{}
	var output *actions.MostRecentlySentOutput
	if isTest {
...
func (h *MostRecentlySentHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
