...
// CreateFolderHandler is a handler
type CreateFolderHandler struct {
	c *connection.Cache
}

// NewCreateFolderHandler returns a new CreateFolderHandler
func NewCreateFolderHandler(c *connection.Cache) *CreateFolderHandler {
...
func (h *CreateFolderHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating CreateFolderInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating CreateFolderInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.CreateFolderAction{}
	var output *actions.CreateFolderOutput
	if isTest {
...
func (h *CreateFolderHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
