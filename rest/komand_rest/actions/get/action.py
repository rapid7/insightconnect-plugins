...
class Get(komand.Action):
    def __init__(self):
        super(self.__class__, self).__init__(
            name="get", description="Make a GET request", input=GetInput(), output=GetOutput()
        )
    ...
    def run(self, params={}):
        ...
        response = requests.get(url, headers=req_headers, verify=self.connection.ssl_verify, auth=self.connection.auth)
        body_object = {}
        try:
