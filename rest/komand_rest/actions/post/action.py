...
class Post(komand.Action):
    def __init__(self):
        super(self.__class__, self).__init__(
            name="post", description="Make a POST request", input=PostInput(), output=PostOutput()
        )
    ...
    def run(self, params={}):
        ...
        response = requests.post(
            url, headers=req_headers, json=body, verify=self.connection.ssl_verify, auth=self.connection.auth
        )
        body_object = {}
        try:
