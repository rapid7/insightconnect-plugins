...
// ValidateHandler is a handler
type ValidateHandler struct {
	c *connection.Cache
}

// NewValidateHandler returns a new ValidateHandler
func NewValidateHandler(c *connection.Cache) *ValidateHandler {
...
func (h *ValidateHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ValidateInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ValidateInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	var output *actions.ValidateOutput
	if isTest {
...
func (h *ValidateHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
