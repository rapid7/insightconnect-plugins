...
import (
	...
	"github.com/rapid7/komand-plugins/numverify/actions"
	"github.com/rapid7/komand-plugins/numverify/connection"
	"github.com/rapid7/komand-plugins/numverify/server/http"
...
var (
	...
	Description = "Look up phone number with NumVerify "
...
func main() {
	...
	case httpServer.FullCommand():
		...
		fmt.Printf("Plugin numverify booting HTTP Daemon on port %d...", *port)
...
func runOneOffAction(mode string, b *message.BodyV1, isDebug bool) error {
	...
	switch b.Action {
	case "validate":
		input := &actions.ValidateInput{}
		if err = json.Unmarshal(b.Input, input); err != nil {
			return err
		}
		for _, err = range input.Validate(l) {
			log.Println(fmt.Sprintf("Error while validating ValidateTriggerInput: %s", err.Error()))
		}
		if err != nil {
			return fmt.Errorf("Error while validating ValidateTriggerInput. Check logs for details")
		}
		a := &actions.ValidateAction{}
		runMethod := a.Run
		if mode == "test" {
			runMethod = a.Test
		}
...
func printInfo() {
	...
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "validate", cli.Reset, "Validate phone number with NumVerify", cli.Reset)
...
func printSampleInput(action string) error {
	...
	case "validate":
		m.Type = "action_start"
		actionBytes, err := json.Marshal(actions.ValidateInput{})
		if err != nil {
			return err
		}
		b.Input = actionBytes
