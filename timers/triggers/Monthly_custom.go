...
// CustomMonthlyTriggerInputParams defines any bonus params needed for the trigger run
type CustomMonthlyTriggerInputParams struct {
...
func (i *MonthlyTriggerInput) Validate(log plog.Logger) []error {
	errors := make([]error, 0)
...
func (o *MonthlyTriggerOutput) Validate(log plog.Logger) []error {
	errors := make([]error, 0)
...
func (t *MonthlyTrigger) Run(conn *connection.Connection, input *MonthlyTriggerInput, log plog.Logger) (*MonthlyTriggerOutput, error) {
	output := &MonthlyTriggerOutput{}
	times, err := parseDailyTimes(input.TimesInUtc)
	dom := input.DayOfMonth
	if err != nil {
		return nil, err
	}
	...
	for {
		sleepUntilNextTime(times, dailyPeriod)
		if time.Now().Day() == dom {
...
func (t *MonthlyTrigger) Test(conn *connection.Connection, input *MonthlyTriggerInput, log plog.Logger) (*MonthlyTriggerOutput, error) {
	output := &MonthlyTriggerOutput{}
	// Your code here
