...
// WeeklyTriggerCustomLooper is an interface you can implement to override RunLoop behavior
type WeeklyTriggerCustomLooper interface {
	RunLoopCustom(ctx context.Context, conn *connection.Connection, input *WeeklyTriggerInput, log plog.Logger) error
}

// WeeklyTriggerInput defines the input for the WeeklyTrigger
type WeeklyTriggerInput struct {
	CustomWeeklyTriggerInputParams
	Day        string `json:"day"`
	Message    string `json:"message"`
	TimesInUtc string `json:"times_in_utc"`
}

// WeeklyTriggerOutput defines the output for the WeeklyTrigger
type WeeklyTriggerOutput struct {
	Message string `json:"message"`
	Time    string `json:"time"`
}

// WeeklyTrigger a trigger
type WeeklyTrigger struct {
	queue      chan *WeeklyTriggerOutput
	dispatcher dispatcher.Dispatcher
	meta       json.RawMessage
}

// NewWeeklyTrigger returns a new WeeklyTrigger properly initialized and ready to run.
func NewWeeklyTrigger(d dispatcher.Dispatcher, meta json.RawMessage) *WeeklyTrigger {
	return &WeeklyTrigger{
		queue:      make(chan *WeeklyTriggerOutput, 1),
		dispatcher: d,
		meta:       meta,
	}
}

// Send will submit a WeeklyTriggerOutput onto the internal queue, which then buffers to the external queue
func (t *WeeklyTrigger) Send(o interface{}) {
	t.queue <- o.(*WeeklyTriggerOutput)
}

// Stop will close the internal queue, which will prevent new jobs from entering.
func (t *WeeklyTrigger) Stop() {
	close(t.queue)
}
...
func (t *WeeklyTrigger) RunLoop(ctx context.Context, conn *connection.Connection, input *WeeklyTriggerInput, log plog.Logger) error {
	...
	for _, err = range input.Validate(log) {
		log.Errorf("Error while validating WeeklyTriggerInput: %s\n", err.Error())
	}
	if err != nil {
		return fmt.Errorf("Error while validating WeeklyTriggerInput. Check logs for details")
	}
	...
	for {
		...
		if err != nil {
			log.Errorf("Error while running trigger action for WeeklyTrigger: %s\n", err.Error())
			continue
		}
...
func (t *WeeklyTrigger) ReadLoop(ctx context.Context, log plog.Logger) error {
	for {
		select {
		case output := <-t.queue:
			if output == nil {
