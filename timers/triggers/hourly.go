...
// HourlyTriggerCustomLooper is an interface you can implement to override RunLoop behavior
type HourlyTriggerCustomLooper interface {
	RunLoopCustom(ctx context.Context, conn *connection.Connection, input *HourlyTriggerInput, log plog.Logger) error
}

// HourlyTriggerInput defines the input for the HourlyTrigger
type HourlyTriggerInput struct {
	CustomHourlyTriggerInputParams
	Message string `json:"message"`
	Minute  int    `json:"minute"`
}

// HourlyTriggerOutput defines the output for the HourlyTrigger
type HourlyTriggerOutput struct {
	Message string `json:"message"`
	Time    string `json:"time"`
}

// HourlyTrigger a trigger
type HourlyTrigger struct {
	queue      chan *HourlyTriggerOutput
	dispatcher dispatcher.Dispatcher
	meta       json.RawMessage
}

// NewHourlyTrigger returns a new HourlyTrigger properly initialized and ready to run.
func NewHourlyTrigger(d dispatcher.Dispatcher, meta json.RawMessage) *HourlyTrigger {
	return &HourlyTrigger{
		queue:      make(chan *HourlyTriggerOutput, 1),
		dispatcher: d,
		meta:       meta,
	}
}

// Send will submit a HourlyTriggerOutput onto the internal queue, which then buffers to the external queue
func (t *HourlyTrigger) Send(o interface{}) {
	t.queue <- o.(*HourlyTriggerOutput)
}

// Stop will close the internal queue, which will prevent new jobs from entering.
func (t *HourlyTrigger) Stop() {
	close(t.queue)
}
...
func (t *HourlyTrigger) RunLoop(ctx context.Context, conn *connection.Connection, input *HourlyTriggerInput, log plog.Logger) error {
	...
	for _, err = range input.Validate(log) {
		log.Errorf("Error while validating HourlyTriggerInput: %s\n", err.Error())
	}
	if err != nil {
		return fmt.Errorf("Error while validating HourlyTriggerInput. Check logs for details")
	}
	...
	for {
		...
		if err != nil {
			log.Errorf("Error while running trigger action for HourlyTrigger: %s\n", err.Error())
			continue
		}
...
func (t *HourlyTrigger) ReadLoop(ctx context.Context, log plog.Logger) error {
	for {
		select {
		case output := <-t.queue:
			if output == nil {
