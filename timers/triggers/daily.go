...
// DailyTriggerCustomLooper is an interface you can implement to override RunLoop behavior
type DailyTriggerCustomLooper interface {
	RunLoopCustom(ctx context.Context, conn *connection.Connection, input *DailyTriggerInput, log plog.Logger) error
}

// DailyTriggerInput defines the input for the DailyTrigger
type DailyTriggerInput struct {
	CustomDailyTriggerInputParams
	Message    string `json:"message"`
	TimesInUtc string `json:"times_in_utc"`
}

// DailyTriggerOutput defines the output for the DailyTrigger
type DailyTriggerOutput struct {
	Message string `json:"message"`
	Time    string `json:"time"`
}

// DailyTrigger a trigger
type DailyTrigger struct {
	queue      chan *DailyTriggerOutput
	dispatcher dispatcher.Dispatcher
	meta       json.RawMessage
}

// NewDailyTrigger returns a new DailyTrigger properly initialized and ready to run.
func NewDailyTrigger(d dispatcher.Dispatcher, meta json.RawMessage) *DailyTrigger {
	return &DailyTrigger{
		queue:      make(chan *DailyTriggerOutput, 1),
		dispatcher: d,
		meta:       meta,
	}
}

// Send will submit a DailyTriggerOutput onto the internal queue, which then buffers to the external queue
func (t *DailyTrigger) Send(o interface{}) {
	t.queue <- o.(*DailyTriggerOutput)
}

// Stop will close the internal queue, which will prevent new jobs from entering.
func (t *DailyTrigger) Stop() {
	close(t.queue)
}
...
func (t *DailyTrigger) RunLoop(ctx context.Context, conn *connection.Connection, input *DailyTriggerInput, log plog.Logger) error {
	...
	for _, err = range input.Validate(log) {
		log.Errorf("Error while validating DailyTriggerInput: %s\n", err.Error())
	}
	if err != nil {
		return fmt.Errorf("Error while validating DailyTriggerInput. Check logs for details")
	}
	...
	for {
		...
		if err != nil {
			log.Errorf("Error while running trigger action for DailyTrigger: %s\n", err.Error())
			continue
		}
...
func (t *DailyTrigger) ReadLoop(ctx context.Context, log plog.Logger) error {
	for {
		select {
		case output := <-t.queue:
			if output == nil {
