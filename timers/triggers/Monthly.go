...
// MonthlyTriggerCustomLooper is an interface you can implement to override RunLoop behavior
type MonthlyTriggerCustomLooper interface {
	RunLoopCustom(ctx context.Context, conn *connection.Connection, input *MonthlyTriggerInput, log plog.Logger) error
}

// MonthlyTriggerInput defines the input for the MonthlyTrigger
type MonthlyTriggerInput struct {
	CustomMonthlyTriggerInputParams
	DayOfMonth int    `json:"day_of_month"`
	Message    string `json:"message"`
	TimesInUtc string `json:"times_in_utc"`
}

// MonthlyTriggerOutput defines the output for the MonthlyTrigger
type MonthlyTriggerOutput struct {
	Message string `json:"message"`
	Time    string `json:"time"`
}

// MonthlyTrigger a trigger
type MonthlyTrigger struct {
	queue      chan *MonthlyTriggerOutput
	dispatcher dispatcher.Dispatcher
	meta       json.RawMessage
}

// NewMonthlyTrigger returns a new MonthlyTrigger properly initialized and ready to run.
func NewMonthlyTrigger(d dispatcher.Dispatcher, meta json.RawMessage) *MonthlyTrigger {
	return &MonthlyTrigger{
		queue:      make(chan *MonthlyTriggerOutput, 1),
		dispatcher: d,
		meta:       meta,
	}
}

// Send will submit a MonthlyTriggerOutput onto the internal queue, which then buffers to the external queue
func (t *MonthlyTrigger) Send(o interface{}) {
	t.queue <- o.(*MonthlyTriggerOutput)
}

// Stop will close the internal queue, which will prevent new jobs from entering.
func (t *MonthlyTrigger) Stop() {
	close(t.queue)
}
...
func (t *MonthlyTrigger) RunLoop(ctx context.Context, conn *connection.Connection, input *MonthlyTriggerInput, log plog.Logger) error {
	...
	for _, err = range input.Validate(log) {
		log.Errorf("Error while validating MonthlyTriggerInput: %s\n", err.Error())
	}
	if err != nil {
		return fmt.Errorf("Error while validating MonthlyTriggerInput. Check logs for details")
	}
	...
	for {
		...
		if err != nil {
			log.Errorf("Error while running trigger action for MonthlyTrigger: %s\n", err.Error())
			continue
		}
...
func (t *MonthlyTrigger) ReadLoop(ctx context.Context, log plog.Logger) error {
	for {
		select {
		case output := <-t.queue:
			if output == nil {
