...
// DelayHandler is a handler
type DelayHandler struct {
	c *connection.Cache
}

// NewDelayHandler returns a new DelayHandler
func NewDelayHandler(c *connection.Cache) *DelayHandler {
	return &DelayHandler{
		c: c,
	}
}
...
func (h *DelayHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating DelayInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating DelayInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.DelayAction{}
	var output *actions.DelayOutput
	if isTest {
...
func (h *DelayHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
