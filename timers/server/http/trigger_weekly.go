...
// WeeklyTriggerHandler is a handler
type WeeklyTriggerHandler struct {
	c *connection.Cache
}

// NewWeeklyTriggerHandler returns a new WeeklyHandler
func NewWeeklyTriggerHandler(c *connection.Cache) *WeeklyTriggerHandler {
	return &WeeklyTriggerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *WeeklyTriggerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating WeeklyInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating WeeklyInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	trigger := triggers.NewWeeklyTrigger(nil, response.Meta)
	var output *triggers.WeeklyTriggerOutput
...
func (h *WeeklyTriggerHandler) replyWithError(response *message.TriggerEvent, w http.ResponseWriter, err error) {
