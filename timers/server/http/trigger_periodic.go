...
// PeriodicTriggerHandler is a handler
type PeriodicTriggerHandler struct {
	c *connection.Cache
}

// NewPeriodicTriggerHandler returns a new PeriodicHandler
func NewPeriodicTriggerHandler(c *connection.Cache) *PeriodicTriggerHandler {
	return &PeriodicTriggerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *PeriodicTriggerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating PeriodicInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating PeriodicInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	trigger := triggers.NewPeriodicTrigger(nil, response.Meta)
	var output *triggers.PeriodicTriggerOutput
...
func (h *PeriodicTriggerHandler) replyWithError(response *message.TriggerEvent, w http.ResponseWriter, err error) {
