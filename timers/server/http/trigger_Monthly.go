...
// MonthlyTriggerHandler is a handler
type MonthlyTriggerHandler struct {
	c *connection.Cache
}

// NewMonthlyTriggerHandler returns a new MonthlyHandler
func NewMonthlyTriggerHandler(c *connection.Cache) *MonthlyTriggerHandler {
	return &MonthlyTriggerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *MonthlyTriggerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating MonthlyInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating MonthlyInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	trigger := triggers.NewMonthlyTrigger(nil, response.Meta)
	var output *triggers.MonthlyTriggerOutput
...
func (h *MonthlyTriggerHandler) replyWithError(response *message.TriggerEvent, w http.ResponseWriter, err error) {
