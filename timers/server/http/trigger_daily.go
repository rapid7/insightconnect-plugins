...
// DailyTriggerHandler is a handler
type DailyTriggerHandler struct {
	c *connection.Cache
}

// NewDailyTriggerHandler returns a new DailyHandler
func NewDailyTriggerHandler(c *connection.Cache) *DailyTriggerHandler {
	return &DailyTriggerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *DailyTriggerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating DailyInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating DailyInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	trigger := triggers.NewDailyTrigger(nil, response.Meta)
	var output *triggers.DailyTriggerOutput
...
func (h *DailyTriggerHandler) replyWithError(response *message.TriggerEvent, w http.ResponseWriter, err error) {
