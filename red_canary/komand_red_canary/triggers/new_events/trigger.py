...
class NewEvents(komand.Trigger):
    ...
    def run(self, params={}):
        ...
        cache_file_name, cache_date = load_cache(self.CACHE_FILE_NAME, self.connection.customer_id, self.logger, force_offset, date_offset)
        ...
        while True:
            try:
                ...
                for event in events:
                    event_date = maya.MayaDT.from_rfc3339(event['attributes']['process']['attributes']['started_at']).datetime()
                ...
                raise Exception(
                    'An error occurred while reading events: {}'.format(e)
                )
