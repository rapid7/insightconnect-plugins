...
class TCell:
    ...
    def create_app(self, app_display_name):
        return self.call_api("POST", "apps", json={"app_display_name": app_display_name})
    ...
    def add_tags(self, app_id, tags):
        try:
            response = self.call_api("PUT", "apps/" + app_id + "/tags", json={"tags": tags})
        except Exception as e:
            self.logger.error("Failed to add tags {} to app {}: {}".format(tags, app_id, str(e)))
            return False
        return response.get("message") == "Request successful"

    def remove_tags(self, app_id, tags):
        try:
            response = self.call_api("DELETE", "apps/" + app_id + "/tags", json={"tags": tags})
        except Exception as e:
            self.logger.error("Failed to remove tags from app {}: {}".format(app_id, str(e)))
            return False
        return response.get("message") == "Request successful"
    ...
    def list_agents(self, app_id, from_=None, to=None, per_page=10, page=1):
        ...
        params = clean(params)
        return self.call_api("GET", "apps/" + app_id + "/agents", params=params, allow_404=True)
    ...
    def list_routes(self, app_id, from_=None, to=None, per_page=10, page=1):
        params = {"from": from_, "to": to, "per_page": per_page, "page": page}
        params = clean(params)
        return self.call_api("GET", "apps/" + app_id + "/routes", params=params, allow_404=True)

    def get_inline_script_details(self, app_id, inline_script_id):
        return self.call_api("GET", "apps/" + app_id + "/inline_scripts/" + inline_script_id)
    ...
    def list_packages(self, app_id, from_=None, to=None, per_page=10, page=1):
        params = {"from": from_, "to": to, "per_page": per_page, "page": page}
        params = clean(params)
        return self.call_api("GET", "apps/" + app_id + "/packages", params=params, allow_404=True)

    def get_active_config(self, app_id):
        return self.call_api("GET", "apps/" + app_id + "/configs/latest")

    def get_config(self, app_id, config_id):
        return self.call_api("GET", "apps/" + app_id + "/configs/" + str(config_id))

    def list_configs(self, app_id, from_=None, to=None, per_page=10, page=1):
        params = {"from": from_, "to": to, "per_page": per_page, "page": page}
        params = clean(params)
        return self.call_api("GET", "apps/" + app_id + "/configs", params=params, allow_404=True)
    ...
    def update_ip_group(self, group_name, items):
        return self.call_api("POST", "ipgroups/" + group_name + "/items", json={"items": items})

    def call_api(self, method, url, params=None, json=None, data=None, allow_404=False):
        ...
        try:
            ...
            if resp.status_code == 401:
                ...
                try:
                    content = resp.json()
                    cause = find_value("message", content) or find_value("error_message", content)
