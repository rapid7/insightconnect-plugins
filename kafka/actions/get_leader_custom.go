...
func (i *GetLeaderInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *GetLeaderOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *GetLeaderAction) Run(conn *connection.Connection, input *GetLeaderInput, log plog.Logger) (*GetLeaderOutput, error) {
	leader, err := conn.Client.Leader(input.Topic, int32(input.Partition))
	if err != nil {
		return &GetLeaderOutput{}, err
	}

	return &GetLeaderOutput{
		Leader: leader.Addr(),
	}, nil
}
...
func (a *GetLeaderAction) Test(conn *connection.Connection, input *GetLeaderInput, log plog.Logger) (*GetLeaderOutput, error) {
