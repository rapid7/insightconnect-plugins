...
func (i *GetBrokersInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *GetBrokersOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *GetBrokersAction) Run(conn *connection.Connection, input *GetBrokersInput, log plog.Logger) (*GetBrokersOutput, error) {
	bs := conn.Client.Brokers()
	brokers := make([]string, len(bs))

	for i, b := range bs {
		brokers[i] = b.Addr()
	}

	return &GetBrokersOutput{
		Brokers: brokers,
	}, nil
}
...
func (a *GetBrokersAction) Test(conn *connection.Connection, input *GetBrokersInput, log plog.Logger) (*GetBrokersOutput, error) {
