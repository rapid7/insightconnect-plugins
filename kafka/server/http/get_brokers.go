...
// GetBrokersHandler is a handler
type GetBrokersHandler struct {
	c *connection.Cache
}

// NewGetBrokersHandler returns a new GetBrokersHandler
func NewGetBrokersHandler(c *connection.Cache) *GetBrokersHandler {
	return &GetBrokersHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *GetBrokersHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating GetBrokersInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating GetBrokersInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.GetBrokersAction{}
	var output *actions.GetBrokersOutput
	if isTest {
...
func (h *GetBrokersHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
