...
// GetTopicsHandler is a handler
type GetTopicsHandler struct {
	c *connection.Cache
}

// NewGetTopicsHandler returns a new GetTopicsHandler
func NewGetTopicsHandler(c *connection.Cache) *GetTopicsHandler {
...
func (h *GetTopicsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating GetTopicsInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating GetTopicsInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.GetTopicsAction{}
	var output *actions.GetTopicsOutput
	if isTest {
...
func (h *GetTopicsHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
