...
// ProduceHandler is a handler
type ProduceHandler struct {
	c *connection.Cache
}

// NewProduceHandler returns a new ProduceHandler
func NewProduceHandler(c *connection.Cache) *ProduceHandler {
...
func (h *ProduceHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ProduceInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ProduceInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ProduceAction{}
	var output *actions.ProduceOutput
	if isTest {
...
func (h *ProduceHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
