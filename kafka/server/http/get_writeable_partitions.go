...
// GetWriteablePartitionsHandler is a handler
type GetWriteablePartitionsHandler struct {
	c *connection.Cache
}

// NewGetWriteablePartitionsHandler returns a new GetWriteablePartitionsHandler
func NewGetWriteablePartitionsHandler(c *connection.Cache) *GetWriteablePartitionsHandler {
	return &GetWriteablePartitionsHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *GetWriteablePartitionsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating GetWriteablePartitionsInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating GetWriteablePartitionsInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.GetWriteablePartitionsAction{}
	var output *actions.GetWriteablePartitionsOutput
	if isTest {
...
func (h *GetWriteablePartitionsHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
