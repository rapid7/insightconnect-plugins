...
// ProduceAsyncHandler is a handler
type ProduceAsyncHandler struct {
	c *connection.Cache
}

// NewProduceAsyncHandler returns a new ProduceAsyncHandler
func NewProduceAsyncHandler(c *connection.Cache) *ProduceAsyncHandler {
	return &ProduceAsyncHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *ProduceAsyncHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ProduceAsyncInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ProduceAsyncInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.ProduceAsyncAction{}
	var output *actions.ProduceAsyncOutput
	if isTest {
...
func (h *ProduceAsyncHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
