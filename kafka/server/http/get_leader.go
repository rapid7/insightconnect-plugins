...
// GetLeaderHandler is a handler
type GetLeaderHandler struct {
	c *connection.Cache
}

// NewGetLeaderHandler returns a new GetLeaderHandler
func NewGetLeaderHandler(c *connection.Cache) *GetLeaderHandler {
	return &GetLeaderHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *GetLeaderHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating GetLeaderInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating GetLeaderInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.GetLeaderAction{}
	var output *actions.GetLeaderOutput
	if isTest {
...
func (h *GetLeaderHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
