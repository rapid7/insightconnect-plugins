...
class DeleteWatchlist(komand.Action):
    ...
    def run(self, params={}):
        ...
        if len(watchlists) > 1 and not force_deletion:
            self.logger.error("Warning: Multiple watchlists with ID {id} found. Stopping. Please enable force deletion "
        ...
        for watchlist in watchlists:
            ...
            self.logger.info("Success: Deleted watchlist {name} with ID {id}".format(name=watchlist.name,
                                                                                     id=watchlist.id))
            return {"success": True}
