...
from komand_redis.actions.list_get import ListGet
...
class TestListGet(TestCase):
    def setUp(self) -> None:
        ...
        def mocked_connector(mocked_redis: MagicMock) -> None:
            self.action = Util.default_connector(ListGet())

        mocked_connector()

    def test_list_get(self):
        input_param = Util.load_json("inputs/list_get.json.exp")
        expect = Util.load_json("expected/list_get_exp.json.exp")
