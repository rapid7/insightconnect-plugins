...
class TestQuarantineMultiple(TestCase):
    ...
    @patch(
        "icon_rapid7_insight_agent.util.graphql_api.api_connection.ApiConnection._get_agent_id", return_value="agent_id"
    )
    def test_quarantine_multiple_success(
        self, agent_id_array, interval, quarantine_state, expected, mock_post, something
    ):
