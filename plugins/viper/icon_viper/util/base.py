...
class ViperBase:
    ...
    def _post(cls, config, url, data, files=None, binary=False):
        headers = {"Authorization": f"Token {config.token}"}
        response = requests.post(config.api_url + url, headers=headers, data=data, files=files)
        return ViperBase.__handle_response(response, binary)
    ...
    def __handle_response(cls, response, binary=False):
        ...
        if response.status_code is not 204:
            try:
                ...
                raise PluginException(
                    cause="Error: Received an unexpected response. Please verify your Viper server "
                    "status and try again. ",
                    assistance="If the issue persists please contact support. "
