    ...
    def run(self, params={}):
        ...
        if malop_features:
            ...
            raise PluginException(
                ...
                assistance=f"Ensure that the Malop: {malop_id} contains a registry key to delete and try again."
            )

        actions_by_machine = []
        for i in targets:
            actions_by_machine.append({
                "targetId": i,
                "actionType": "DELETE_REGISTRY_KEY"
            })

        return {Output.RESPONSE: self.connection.api.remediate(
