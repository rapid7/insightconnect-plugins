...
class ConvertToDocx(HTMLConverterStrategy):
    def convert(self, input_html_string: str) -> str:
        return convert_with_temporary_file(
            input_html_string, "docx", "html", "temp_html_2_docx.docx", sandbox=is_running_in_cloud()
        )
...
class ConvertToEpub(HTMLConverterStrategy):
    def convert(self, input_html_string: str) -> str:
        return convert_with_temporary_file(
            input_html_string, "epub", "html", "temp_html_3_epub.epub", sandbox=is_running_in_cloud()
        )
...
class ConvertToPDF(HTMLConverterStrategy):
    def convert(self, input_html_string: str) -> str:
        return convert_with_temporary_file(
            input_html_string, "pdf", "html", "temp_html_2_pdf.pdf", sandbox=is_running_in_cloud()
        )
