...
from subprocess import Popen, SubprocessError  # noqa: B404
import validators
...
class SqlmapApi:
    ...
    def check_server_availability(self) -> None:
        self._call_api("GET", f"{self.base_url}/version")
    ...
    def _wait_for_completion(self, task_id: str) -> None:
        ...
        for _ in range(MAX_WAIT_FOR_COMPLETION_ITERATIONS):
    ...
    def _call_api(self, method: str, url: str, headers: dict[str, str] = None, payload: dict[str, Any] = None) -> Dict:
        try:
            ...
            raise PluginException(
                cause=f"SQLMap API returned HTTP {error.response.status_code}.",
                assistance="Please check the SQLMap API server status and request parameters.",
                data=error,
            )
    ...
    def _initialize_server(self) -> None:
        ...
        if self.api_host and self.api_port:
            command.extend(
                [
                    f"--host={quote(str(self.api_host))}",
                    f"--port={quote(str(self.api_port))}",
                ]
            )

        # Run SQL map server as a separate process
        try:
            Popen(command, stdout=self._logs, stderr=self._logs)  # noqa: B603 # pylint: disable=consider-using-with
    ...
    def get_logs(self, cleanup: bool = False) -> None:
        ...
        for log_line in self._logs.readlines():
            self.logger.info(f"* {log_line.rstrip()}")

        # If cleanup is set to True, just clear the buffer
        if cleanup:
            self._clean_logs()
    ...
    def _validate_host_port(self) -> None:
        ...
        if not self.api_host == "localhost" and not validators.ipv4(self.api_host, cidr=False):
            raise PluginException(
