...
from icon_threatminer.actions.ssl_report import SslReport
from icon_threatminer.actions.ssl_report.schema import Input, Output


class TestSslReport(TestCase):
    def setUp(self) -> None:
        self.action = Util.default_connector(SslReport())
        ...
        [
            (
                {Input.QUERY: "42a8d5b3a867a59a79f44ffadd61460780fe58f2"},
                {
                    ...
                    "results": [{"filename": "ExampleFilename", "year": "2010"}],
                },
            )
        ]
    )
    ...
    def test_ssl_report_exception(self, mock_request: Callable, cause: str, assistance: str) -> None:
