...
from icon_threatminer.actions.av_sample import AvSample
from icon_threatminer.actions.av_sample.schema import Input, Output


class TestAvSample(TestCase):
    def setUp(self) -> None:
        self.action = Util.default_connector(AvSample())

    @parameterized.expand(
        [
            (
                {Input.QUERY: "ExampleQuery"},
                {
                    "status_code": 200,
                    "status_message": "Results found.",
                    "results": [{"value": "f1b341d3383b808ecfacfa22dcbe9196"}],
                },
            )
        ]
    )
    ...
    def test_av_sample_exception(self, mock_request: Callable, cause: str, assistance: str) -> None:
