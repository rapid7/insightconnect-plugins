...
def mock_request(*args, **_kwarg):
    return mock_request_selection(args[0], args[1], _kwarg.get("params"), _kwarg.get("headers") )
...
def mock_request_post(url, params, headers):
...
def mock_request_get(url, params, headers):
...
def mock_request_selection(method, url, params, headers):
    # Check reqeust type and endpoint. Return appropriate file name to be loaded and response code
    if method == REQUEST_POST:
        return mock_request_post(url, params, headers)
    elif method == REQUEST_GET:
        return mock_request_get(url, params, headers)
    raise Exception("Response has been not implemented")
