...
from icon_azure_compute.actions.stop_deallocate_vm import StopDeallocateVm
from icon_azure_compute.actions.stop_deallocate_vm.schema import Input, Output
...
class TestStopDeallocateVm(TestCase):
    @patch("requests.post", side_effect=mock_request_200)
    def setUp(self, mock_requests: MagicMock) -> None:
        self.action = Util.default_connector(StopDeallocateVm())

    @patch("requests.post", side_effect=mock_request_202)
    def test_stop_deallocate_vm(self, mock_request: MagicMock) -> None:
    ...
    def test_stop_deallocate_vm_exception(
