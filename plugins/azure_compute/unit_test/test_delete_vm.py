...
from icon_azure_compute.actions.delete_vm import DeleteVm
from icon_azure_compute.actions.delete_vm.schema import Input, Output
...
class TestDeleteVm(TestCase):
    @patch("requests.post", side_effect=mock_request_200)
    def setUp(self, mock_requests: MagicMock) -> None:
        self.action = Util.default_connector(DeleteVm())

    @patch("requests.delete", side_effect=mock_request_202)
    def test_delete_vm(self, mock_request: MagicMock) -> None:
