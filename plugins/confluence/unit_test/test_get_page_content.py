...
class TestGetPageContent(TestCase):
    ...
    def test_get_page_content(self, page: str, space: str, expected_filename: str, mock_get_id: Mock, mock_get_page: Mock) -> None:
    ...
    @parameterized.expand(
        [
            ("Test Page", "Test Not Found")
        ]
    )
    @mock.patch("atlassian.Confluence.get_page_id",side_effect=Util.mocked_requests)
    @mock.patch("atlassian.Confluence.get_page_by_id", side_effect=Util.mocked_requests)
    def test_get_page_content_failure(self, page: str, space: str, mock_get_id: Mock, mock_get_page: Mock) -> None:
