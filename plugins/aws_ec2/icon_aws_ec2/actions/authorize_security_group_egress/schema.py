...
class Component:
    DESCRIPTION = "Add one or more egress rules to a security group for use with a VPC [EC2-VPC only]. See [http://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_AuthorizeSecurityGroupEgress.html](http://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_AuthorizeSecurityGroupEgress.html)"
...
class AuthorizeSecurityGroupEgressInput(insightconnect_plugin_runtime.Input):
    schema = json.loads(
        r"""
   {
  "type": "object",
...
class AuthorizeSecurityGroupEgressOutput(insightconnect_plugin_runtime.Output):
    schema = json.loads(
        r"""
   {
  "type": "object",
