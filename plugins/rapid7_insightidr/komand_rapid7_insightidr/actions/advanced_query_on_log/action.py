...
class AdvancedQueryOnLog(insightconnect_plugin_runtime.Action):
    ...
    def run(self, params={}):
        ...
        self.logger.info(f"Executing Advanced Query on Log action with Request ID: {request_id}")
        ...
        if not log_id:
            log_id = self.get_log_id(log_name, request_id)
        ...
        callback_url, log_entries = self.maybe_get_log_entries(
            log_id, query, time_from, time_to, statistical, request_id
        )
    ...
    def maybe_get_log_entries(
        ...
        response = send_session_request(
            request_url=endpoint, request_headers=self.connection.headers, request_params=params, request_id=request_id
        )
        try:
            response.raise_for_status()
        except Exception:
    ...
    def get_log_id(self, log_name: str, request_id: str) -> str:
        """
