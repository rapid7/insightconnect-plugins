...
class TestListAttachments(TestCase):
    ...
    def test_list_attachments(self, mock_request, name, target, index, size, expected) -> None:
    ...
    def test_list_attachments_bad(self, mock_request, name, target, index, size, cause, assistance) -> None:
        test_input = {Input.TARGET: target, Input.INDEX: index, Input.SIZE: size}
        validate(test_input, ListAttachmentsInput.schema)
