...
class InputDataValidator:
    ...
    EXCEPTION_MESSAGE_LIMIT = 'Limit of maximum input characters for parameter "{parameter_name}" has been exceeded (maximum {type} {maximum})'
    ...
    def _is_valid_input(self, value: str, max_char: int, parameter_name: str, type: str) -> None:
        if self._check_params_limit(value, max_char):
            raise PluginException(
                preset=PluginException.Preset.UNKNOWN, data=self._exception_message(parameter_name, type, max_char)
            )

    def _check_params_limit(self, value: str, max_char: int) -> bool:
        return max_char < len(value)

    def _exception_message(self, parameter_name: str, type: str, max_char: int) -> str:
        return self.EXCEPTION_MESSAGE_LIMIT.format(parameter_name=parameter_name, type=type, maximum=max_char)
