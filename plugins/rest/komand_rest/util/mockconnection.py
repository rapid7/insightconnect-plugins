...
class MockConnectionApi:
    ...
    def fill_response_obj_test(self, method: str, path: str, data: str = None, json_data: dict = None, headers: dict = None
    ) -> Response:
        ...
        if path == "https://www.google.com":
            response = MockResponse(
                {"SampleSuccessBody": "SampleVal"}, 200,
                "SAMPLETEXT for method " + method)
            response.headers = {"SampleHeader": "SampleVal"}
            return response
        else:
            response = MockResponse(
                {"SampleBodyError": "SampleVal"}, 404,
                "TRYGOOGLE for method " + method)
