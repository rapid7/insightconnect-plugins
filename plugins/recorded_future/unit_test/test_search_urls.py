...
from komand_recorded_future.actions.search_urls import SearchUrls
...
class TestSearchUrls(TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.action = Util.default_connector(SearchUrls())

    @parameterized.expand(
        [
            [
                Util.read_file_to_dict("inputs/search_urls.json.resp"),
                Util.read_file_to_dict("expected/search_urls.json.resp"),
            ],
        ]
    )
    ...
    def test_search_urls_raise_exception(
        self, mock_request: MagicMock, token: str, input_parameters: Dict[str, Any], cause: str, assistance: str
    ) -> None:
        action = Util.default_connector(SearchUrls(), {Input.API_KEY: {"secretKey": token}})
