...
from komand_recorded_future.actions.search_entity_lists import SearchEntityLists
...
class TestSearchEntityLists(TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.action = Util.default_connector(SearchEntityLists())

    @parameterized.expand(
        [
            [
                Util.read_file_to_dict("inputs/search_entity_lists.json.resp"),
                Util.read_file_to_dict("expected/search_entity_lists.json.resp"),
            ],
        ]
    )
    def test_search_entity_lists(
    ...
    def test_search_entity_lists_raise_exception(
        self, mock_request: MagicMock, token: str, input_parameters: Dict[str, Any], cause: str, assistance: str
    ) -> None:
        action = Util.default_connector(SearchEntityLists(), {Input.API_KEY: {"secretKey": token}})
