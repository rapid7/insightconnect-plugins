...
from komand_recorded_future.actions.lookup_hash import LookupHash
...
class TestLookupHash(TestCase):
    ...
    @parameterized.expand(
        [
            [
                Util.read_file_to_dict("inputs/lookup_hash_found.json.resp"),
                Util.read_file_to_dict("expected/lookup_hash_found.json.resp"),
            ],
            [
                Util.read_file_to_dict("inputs/lookup_hash_not_found.json.resp"),
                Util.read_file_to_dict("expected/lookup_hash_not_found.json.resp"),
            ],
            [
                Util.read_file_to_dict("inputs/lookup_hash_invalid_hash_not_found.json.resp"),
                Util.read_file_to_dict("expected/lookup_hash_not_found.json.resp"),
            ],
        ]
    )
    def test_lookup_hash(self, mock_request, input_parameters, expected):
    ...
    def test_hash_alert_raise_exception(self, mock_request, token, input_parameters, cause, assistance):
        action = Util.default_connector(LookupHash(), {Input.API_KEY: {"secretKey": token}})
