...
from komand_recorded_future.actions.download_hash_risk_list import DownloadHashRiskList
...
class TestDownloadHashRiskList(TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.action = Util.default_connector(DownloadHashRiskList())

    @parameterized.expand(
        [
            [
                Util.read_file_to_dict("inputs/hash_risk_list.json.resp"),
                Util.read_file_to_dict("expected/hash_risk_list.json.resp"),
            ],
        ]
    )
    def test_download_hash_risk_list(
    ...
    def test_hash_risk_list_raise_exception(
        self, mock_request: MagicMock, token: str, input_parameters: Dict[str, Any], cause: str, assistance: str
    ) -> None:
        action = Util.default_connector(DownloadHashRiskList(), {Input.API_KEY: {"secretKey": token}})
