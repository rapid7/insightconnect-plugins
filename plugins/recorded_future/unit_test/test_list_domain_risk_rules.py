...
from komand_recorded_future.actions.list_domain_risk_rules import ListDomainRiskRules
...
class TestListDomainRiskRules(TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.action = Util.default_connector(ListDomainRiskRules())

    @parameterized.expand(
        [
            [
                Util.read_file_to_dict("expected/list_domain_risk_rules.json.resp"),
            ],
        ]
    )
    def test_list_domain_risk_rules(self, mock_request: MagicMock, expected: Dict[str, Any]) -> None:
    ...
    def test_list_domain_risk_rules_raise_exception(
        self, mock_request: MagicMock, token: str, input_parameters: Dict[str, Any], cause: str, assistance: str
    ) -> None:
        action = Util.default_connector(ListDomainRiskRules(), {Input.API_KEY: {"secretKey": token}})
