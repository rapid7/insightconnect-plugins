...
from komand_recorded_future.actions.list_hash_risk_rules import ListHashRiskRules
...
class TestListHashRiskRules(TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.action = Util.default_connector(ListHashRiskRules())

    @parameterized.expand(
        [
            [
                Util.read_file_to_dict("expected/list_hash_risk_rules.json.resp"),
            ],
        ]
    )
    def test_list_hash_risk_rules(self, mock_request: MagicMock, expected: Dict[str, Any]) -> None:
    ...
    def test_list_hash_risk_rules_raise_exception(
        self, mock_request: MagicMock, token: str, input_parameters: Dict[str, Any], cause: str, assistance: str
    ) -> None:
        action = Util.default_connector(ListHashRiskRules(), {Input.API_KEY: {"secretKey": token}})
