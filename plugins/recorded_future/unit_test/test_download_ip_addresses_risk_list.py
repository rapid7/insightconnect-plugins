...
from komand_recorded_future.actions.download_ip_addresses_risk_list import DownloadIpAddressesRiskList
...
class TestDownloadIpAddressesRiskList(TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.action = Util.default_connector(DownloadIpAddressesRiskList())

    @parameterized.expand(
        [
            [
                Util.read_file_to_dict("inputs/ip_risk_list.json.resp"),
                Util.read_file_to_dict("expected/ip_risk_list.json.resp"),
            ],
        ]
    )
    def test_download_ip_addresses_risk_list(
    ...
    def test_ip_addresses_risk_list_raise_exception(
        self, mock_request: MagicMock, token: str, input_parameters: Dict[str, Any], cause: str, assistance: str
    ) -> None:
        action = Util.default_connector(DownloadIpAddressesRiskList(), {Input.API_KEY: {"secretKey": token}})
