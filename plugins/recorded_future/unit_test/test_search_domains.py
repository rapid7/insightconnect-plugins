...
from komand_recorded_future.actions.search_domains import SearchDomains
...
class TestSearchDomains(TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.action = Util.default_connector(SearchDomains())

    @parameterized.expand(
        [
            [
                Util.read_file_to_dict("inputs/search_domains.json.resp"),
                Util.read_file_to_dict("expected/search_domains.json.resp"),
            ],
        ]
    )
    ...
    def test_search_domains_raise_exception(
        self, mock_request: MagicMock, token: str, input_parameters: Dict[str, Any], cause: str, assistance: str
    ) -> None:
        action = Util.default_connector(SearchDomains(), {Input.API_KEY: {"secretKey": token}})
