...
from komand_recorded_future.actions.search_hashes import SearchHashes
...
class TestSearchHashes(TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.action = Util.default_connector(SearchHashes())

    @parameterized.expand(
        [
            [
                Util.read_file_to_dict("inputs/search_hashes.json.resp"),
                Util.read_file_to_dict("expected/search_hashes.json.resp"),
            ],
        ]
    )
    ...
    def test_search_hashes_raise_exception(
        self, mock_request: MagicMock, token: str, input_parameters: Dict[str, Any], cause: str, assistance: str
    ) -> None:
        action = Util.default_connector(SearchHashes(), {Input.API_KEY: {"secretKey": token}})
