...
* 5.1.0 - Handle 404 Not Found Error in `Lookup IP`, `Lookup Domain`, `Lookup Hash`, `Lookup Alert`, `Lookup Vulnerability`, `Lookup URL`, `Lookup Malware` actions | Create unit_tests for `Lookup IP`, `Lookup Domain`, `Lookup Hash`, `Lookup Alert`, `Lookup Vulnerability`, `Lookup URL`, `Lookup Malware` actions | Update example outputs in help.md | Update keywords | Update key features* 5.0.1 - Update error handling around a domain that is not found in Lookup Domain
