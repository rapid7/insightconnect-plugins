...
class TestGetSubmissionsIDs(TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.action = Util.default_connector(GetSubmissionsIDs())

    @parameterized.expand(
        [
            [
                "valid_parameters",
                Util.read_file_to_dict("inputs/get_submissions_ids.json.inp"),
                Util.read_file_to_dict("expected/get_submissions_ids.json.exp"),
            ]
        ]
    )
    ...
    @parameterized.expand(
        [
            [
                "invalid_filter",
                Util.read_file_to_dict("inputs/get_submissions_ids_invalid_filter.json.inp"),
