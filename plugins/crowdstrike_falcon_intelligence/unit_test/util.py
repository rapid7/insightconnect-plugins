...
class Util:
    ...
    def mock_request(**kwargs):
        ...
        if url == f"{DEFAULT_CONNECTION_PARAMS.get(Input.BASEURL)}/oauth2/token":
            return MockResponse(201, "auth_token.json.resp")
        if url == f"{DEFAULT_CONNECTION_PARAMS.get(Input.BASEURL)}/falconx/entities/submissions/v1" and method == "GET":
        ...
        if url == f"{DEFAULT_CONNECTION_PARAMS.get(Input.BASEURL)}/falconx/entities/artifacts/v1" and method == "GET":
            if params.get("id").startswith("404"):
                return MockResponse(500, "")
            else:
                return MockResponse(200, "download_artifact.json.resp")
        if url == f"{DEFAULT_CONNECTION_PARAMS.get(Input.BASEURL)}/falconx/entities/reports/v1" and method == "GET":
            if params.get("ids")[0].startswith("404"):
        ...
        if url == f"{DEFAULT_CONNECTION_PARAMS.get(Input.BASEURL)}/falconx/queries/reports/v1" and method == "GET":
        ...
        if (
            url == f"{DEFAULT_CONNECTION_PARAMS.get(Input.BASEURL)}/falconx/entities/report-summaries/v1"
            and method == "GET"
        ):
            if params.get("ids")[0].startswith("404"):
        ...
        if url == f"{DEFAULT_CONNECTION_PARAMS.get(Input.BASEURL)}/falconx/queries/submissions/v1" and method == "GET":
        ...
        if (
            url == f"{DEFAULT_CONNECTION_PARAMS.get(Input.BASEURL)}/falconx/entities/submissions/v1"
            and method == "POST"
        ):
            return MockResponse(200, "submit_analysis.json.resp")
        if url == f"{DEFAULT_CONNECTION_PARAMS.get(Input.BASEURL)}/samples/entities/samples/v2" and method == "POST":
            return MockResponse(200, "upload_malware_sample.json.resp")
