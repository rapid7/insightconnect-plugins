...
class TestSearchIncidents(TestCase):
    ...
    def test_search_incidents(self, _mock_req):
        actual = self.action.run(
            {Input.OBJECT_TYPE: self.params.get("object_type"), Input.SKIP: self.params.get("skip"),
             Input.TEXT: self.params.get("good_text"), Input.TOP: self.params.get("top")})
    ...
    def test_search_incidents_fail(self, _mock_req):
        with self.assertRaises(PluginException) as exception:
            self.action.run({Input.OBJECT_TYPE: self.params.get("object_type"), Input.SKIP: self.params.get("skip"),
                             Input.TEXT: self.params.get("bad_text"), Input.TOP: self.params.get("top")})
