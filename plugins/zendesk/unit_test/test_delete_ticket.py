...
from komand_zendesk.actions.delete_ticket import DeleteTicket
from komand_zendesk.actions.delete_ticket.schema import Input, Output
...
class TestDeleteTicket(TestCase):
    ...
    def test_delete_ticket(self, mock_request, mock_second):
    ...
    @patch("zenpy.TicketApi.delete", side_effect=Util.mocked_requests)
    @patch("zenpy.TicketApi.__call__", side_effect=Util.mocked_requests)
    def test_delete_ticket_fail(self, mock_request, mock_second):
