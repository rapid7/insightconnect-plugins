...
def call_api_and_validate_response(
    method, endpoint, error_message="Invalid API call", logger=None, auth=None, *args, **kwargs
):
    if logger is None:
    ...
    try:
        ...
        logger.info("CallAPI: API call successful, but response is not of type JSON")
        return response
