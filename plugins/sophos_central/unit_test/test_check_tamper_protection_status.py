...
from icon_sophos_central.actions.check_tamper_protection_status import CheckTamperProtectionStatus

sys.path.append(os.path.abspath("../"))


class TestCheckTamperProtectionStatus(TestCase):
    @classmethod
    def setUpClass(cls):
        cls.action = Util.default_connector(CheckTamperProtectionStatus())

    @patch("requests.request", side_effect=Util.mock_request)
    @patch("requests.request", side_effect=Util.mock_request)
    def test_check_tamper_protection_status(self, mock_request_1, mock_request_2):
        input_params = Util.read_file_to_dict("inputs/get_endpoint_id.json.inp")
        expected = Util.read_file_to_dict("expected/check_tamper_protection_status.json.exp")
    ...
    def test_check_tamper_protection_status_raise_exception(self, cause, assistance, mock_request_1):
