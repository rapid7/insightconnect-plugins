...
* Cyber GRC enforces a character limit on many text fields, and the limit varies by field: a comment is 5000 characters, a risk description, business impact and possible outcome are 2000 each, an IT asset description is 4000, an audit description and an assessment summary are 2000, and several fields such as an assessment description, a vendor description, and most incident narrative fields are 500 (an incident's lessons learned and a task description are 1000). Rather than fail the step, the plugin truncates a value that is over its field's limit and records a warning in the job log naming the field and the lengths involved, followed by the full original value so nothing is lost from the log. To keep the full text in the record, shorten the value in the workflow before this step
