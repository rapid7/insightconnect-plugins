...
class TestActionResetPassword(TestCase):
    ...
    def test_reset_password(self, action):
        actual = action.run({Input.DISTINGUISHED_NAME: "CN=Users,DC=example,DC=com", Input.NEW_PASSWORD: "test_pass"})
    ...
    def test_reset_password_raise(self, action):
        with self.assertRaises(PluginException) as context:
            action.run(
                {Input.DISTINGUISHED_NAME: "CN=LDAPInvalidDnError,DC=example,DC=com", Input.NEW_PASSWORD: "test_pass"}
            )

        self.assertEqual("LDAP returned an error in the response.", context.exception.cause)
        self.assertEqual("LDAP failed to reset the password for this user", context.exception.assistance)
    ...
    def test_reset_password_raise_when_ssl_false(self, action):
        ...
        self.assertEqual("SSL must be enabled for the reset password action", context.exception.assistance)
