...
KEYS_TO_REMOVE = ["@odata.context", "redirectIncidentId"]
...
class MicrosoftDefenderClientAPI(AzureClient):
    ...
    @request_execution_time
    @_connection
    def list_incidents(self, params: dict = None) -> List[dict]:
        response = self._call_api("GET", Endpoint.LIST_INCIDENTS, params=params).get("value", [])
        response = [return_non_empty(element) for element in response]
        return response

    @_connection
    def get_incident(self, identifier: int) -> dict:
        return fill_required_fields(
            return_non_empty(
                remove_keys_from_dict(
                    self._call_api("GET", Endpoint.GET_INCIDENT.format(identifier)),
                    KEYS_TO_REMOVE,
                )
            )
        )
    ...
    @_connection
    def update_incident(self, identifier: int, json_body: dict) -> dict:
        return fill_required_fields(
            remove_keys_from_dict(
                return_non_empty(
                    self._call_api(
                        "PATCH",
                        Endpoint.UPDATE_INCIDENT.format(identifier),
                        json_data=json_body,
                    )
                ),
                KEYS_TO_REMOVE,
