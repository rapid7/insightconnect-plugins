...
from icon_microsoft_defender_incidents.util.endpoints import Endpoint

STUB_CONNECTION = {"client_id": "1234", "client_secret": {"secretKey": "1234"}, "tenant_id": "123"}
STUB_INCIDENT_IDENTIFIER = 924518
...
def mock_conditions(method: str, url: str, status_code: int) -> MockResponse:
    ...
    if url == Endpoint.LIST_INCIDENTS:
        return MockResponse("test_list_incidents", status_code)
    if url == Endpoint.GET_INCIDENT.format(STUB_INCIDENT_IDENTIFIER):
        if method == "GET":
            return MockResponse("test_get_incident", status_code)
        return MockResponse("test_update_incident", status_code)
