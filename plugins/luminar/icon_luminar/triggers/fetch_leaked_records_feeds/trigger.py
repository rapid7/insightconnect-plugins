...
class FetchIocFeeds(insightconnect_plugin_runtime.Trigger):
    ...
    def run(self, params={}):
        ...
        while True:
            ...
            ioc_collection_id = self.connection.client.get_taxi_collections().get("leakedrecords")
            if ioc_collection_id:
                self.logger.info(f"Fetching leakedrecords feeds from Luminar....")
                ioc_records = self.connection.client.get_collection_objects(ioc_collection_id, params)
                if ioc_records:
                    self.logger.info(f"Fetched leakedrecords records.")
                    ...
                    if filter_records:
                        ...
                        self.logger.info("No new leakedrecords records found.")
                else:
                    self.logger.info("No leakedrecords records found.")
            else:
                self.logger.info("No leakedrecords collection found.")
