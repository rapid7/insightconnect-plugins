    ...
    def run(self, params={}):
        ...
        try:
            ...
            raise PluginException(
                cause=f"There was an issue with the Get Subscribed SKUs user request. Double-check the username: {user_principal_name}",
                data=response.text,
            )
        if response.status_code == 200:
            ...
            PluginException(
                cause=f"The response from Office365 indicated something went wrong: {response.status_code}",
                data=response.text,
            )
