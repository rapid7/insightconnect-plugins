...
from icon_trendmicro_visionone.actions.download_sandbox_investigation_package import (
    DownloadSandboxInvestigationPackage,
)
...
class TestDownloadSandboxInvestigationPackage(TestCase):
    ...
    def test_integration_download_sandbox_investigation_package(self):
        ...
        test_action = DownloadSandboxInvestigationPackage()
        ...
        try:
            with open(
                "/python/src/tests/download_sandbox_investigation_package.json"
            ) as file:
    ...
    @patch("pytmv1.client")
    def test_download_sandbox_investigation_package_success(self, mock_pytmv1_client):
        mock_client_instance = MagicMock()
        mock_response = MagicMock(
            response=MagicMock(content=base64.b64encode(b"File content"))
        )
        mock_client_instance.download_sandbox_investigation_package.return_value = (
            mock_response
        )
    ...
    @patch("pytmv1.client")
    def test_download_sandbox_investigation_package_failure(self, mock_pytmv1_client):
        mock_client_instance = MagicMock()
        mock_response = MagicMock(
            result_code="Error",
