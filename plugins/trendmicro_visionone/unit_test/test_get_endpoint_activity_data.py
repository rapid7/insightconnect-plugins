...
class TestGetEndpointActivityData(TestCase):
    ...
    def test_1_integration_get_endpoint_activity_data(self):
        ...
        self.action.connection.client.endpoint.get_activity_count = MagicMock(return_value=mock_activity_count_response)

        # Mocking consume_activity response
        def consume_activity(callback, **kwargs):
            endpoint_activity_data = MockResponse(total_count=100, result_code="success", data=expected_result)
            callback(endpoint_activity_data.response)
            return MockResponse(total_count=100, result_code="success")

        self.action.connection.client.endpoint.consume_activity = consume_activity
