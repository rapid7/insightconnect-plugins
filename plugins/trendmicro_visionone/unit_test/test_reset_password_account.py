...
from icon_trendmicro_visionone.actions.reset_password_account import (
...
class TestResetPasswordAccount(TestCase):
    ...
    def test_integration_reset_password_account(self):
        ...
        test_action = ResetPasswordAccount()

        test_conn.logger = log
        test_action.logger = log

        try:
            with open("/python/src/tests/reset_password_account.json") as file:
        ...
        expected_output = {"multi_response": [{"status": 202, "task_id": "00000011"}]}

        self.assertEqual(results, expected_output)

    @patch("pytmv1.client")
    def test_reset_password_account_success(self, mock_pytmv1_client):
        mock_client_instance = MagicMock()
        mock_client_instance.reset_password_account.side_effect = [
            MagicMock(
                result_code="Success",
                response=MagicMock(
                    dict=MagicMock(
                        return_value={"items": [{"status": 202, "task_id": "00000011"}]}
                    )
                ),
            ),
        ]
    ...
    @patch("pytmv1.client")
    def test_reset_password_account_failure(self, mock_pytmv1_client):
        mock_client_instance = MagicMock()
        mock_client_instance.reset_password_account.side_effect = Exception(
            "API request failed"
        )
        mock_pytmv1_client.return_value = mock_client_instance
