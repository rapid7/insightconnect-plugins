...
class TestDownloadSandboxAnalysisResult(TestCase):
    ...
    def test_download_sandbox_analysis_result_success(self):
        ...
        self.action.connection.client.download_sandbox_analysis_result = MagicMock(return_value=mock_response)
        response = self.action.run(self.mock_params["input"])
        for key in response.keys():
            self.assertIn(key, str(expected_result.keys()))

    def test_download_sandbox_analysis_result_failure(self):
        self.action.connection.client.download_sandbox_analysis_result = MagicMock(side_effect=PluginException)
