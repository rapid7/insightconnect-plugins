...
from icon_trendmicro_visionone.actions.remove_from_exception_list import (
...
class TestRemoveFromExceptionList(TestCase):
    ...
    def test_integration_remove_from_exception_list(self):
        ...
        test_action = RemoveFromExceptionList()

        test_conn.logger = log
        test_action.logger = log

        try:
            with open("/python/src/tests/remove_from_exception_list.json") as file:
    ...
    @patch("pytmv1.client")
    def test_remove_from_exception_list_success(self, mock_pytmv1_client):
        mock_client_instance = MagicMock()
        mock_client_instance.remove_from_exception_list.side_effect = [
            MagicMock(
                result_code="Success",
                response=MagicMock(
                    dict=MagicMock(
        ...
        params = {"block_object": [{"object_type": "ip", "object_value": "1.6.6.3"}]}
    ...
    @patch("pytmv1.client")
    def test_remove_from_exception_list_failure(self, mock_pytmv1_client):
        mock_client_instance = MagicMock()
        mock_client_instance.remove_from_exception_list.side_effect = Exception(
            "API request failed"
        )
        mock_pytmv1_client.return_value = mock_client_instance
