...
class DownloadOpeniocFile(insightconnect_plugin_runtime.Action):
    ...
    def run(self, params={}):
        quoted_param = urllib.parse.quote(json.dumps({"FileHashID": params.get(Input.FILE_HASH_ID)}))
        return self.connection.api.execute("get", f"/WebApp/IOCBackend/OpenIOCResource/File?param={quoted_param}", "")
