...
class Info(komand.Action):
    ...
    def run(self, params={}):
        ...
        package_list = (
            self.rpm_helper.list_package(label, arch, distro, repo_dic["ids"])
            if repo_dic
            else self.rpm_helper.list_package(label, arch, distro)
        )
        ...
        package_path = (
            self.rpm_helper.download_package(label, arch, distro, package_list, repo_dic["ids"])
            if repo_dic
            else self.rpm_helper.download_package(label, arch, distro, package_list, None)
        )
        if key:
