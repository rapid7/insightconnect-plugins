...
class Util:
    ...
    def mock_request(*args, **kwargs):
        ...
        elif url == "https://example.com/api/v1/groups/12345/users" and not first_request:
            first_request = True
            return MockResponse(200, "get_users_in_group2.json.resp")
        if url == "https://example.com/api/v1/groups/12345":
            return MockResponse(200, "get_group.json.resp")
        if url == "https://example.com/api/v1/users/user@example.com":
            return MockResponse(200, "get_user.json.resp")
        if url == "https://example.com/api/v1/users/invalid@example.com":
            return MockResponse(404)
        if method == "DELETE":
            if url == "https://example.com/api/v1/groups/123456/users/12345":
                return MockResponse(204)
            if url == "https://example.com/api/v1/groups/invalid_group/users/12345":
                return MockResponse(404)
        if url == "https://example.com/api/v1/users/12345/lifecycle/reset_factors":
            return MockResponse(200, "reset_factors.json.resp")
        if url == "https://example.com/api/v1/users/12345/lifecycle/expire_password":
        ...
        if url == "https://example.com/api/v1/users/invalid_user/lifecycle/expire_password":
            return MockResponse(404)
        if url == "https://example.com/api/v1/zones":
            return MockResponse(200, "get_zones.json.resp")
        if method == "PUT" and url == "https://example.com/api/v1/zones/56789":
        ...
        if method == "POST" and url == "https://example.com/api/v1/apps/123456/users":
            if kwargs.get("json") == {"id": "invalid"}:
                return MockResponse(404)
            return MockResponse(200, "assign_user_to_app_sso.json.resp")
        if method == "POST" and url == "https://example.com/api/v1/apps/invalid/users":
            return MockResponse(404)
        if url == "https://example.com/api/v1/groups":
            if params == {"q": "Test"}:
        ...
        if url == "https://example.com/api/v1/users":
        ...
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S1/factors":
            return MockResponse(200, "get_factors_existing_user.json.resp")
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S2/factors":
            return MockResponse(200, "get_factors_existing_user_without_factors.json.resp")
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/factors":
            return MockResponse(404)

        if url == "https://example.com/api/v1/groups/00g1a1qwertYuiopA0h1/users/00u1m1qwertYUiopA0S1":
            return MockResponse(204)
        if url == "https://example.com/api/v1/groups/00g1a1qwertYuiopA0h2/users/00u1m1qwertYUiopA0S1":
            return MockResponse(404)
        if url == "https://example.com/api/v1/groups/00g1a1qwertYuiopA0h1/users/00u1m1qwertYUiopA0S2":
            return MockResponse(204)
        if url == "https://example.com/api/v1/groups/00g1a1qwertYuiopA0h1/users/00u1m1qwertYUiopA0S3":
            return MockResponse(404)

        if url == "https://example.com/api/v1/users/user1@example.com" and method == "GET":
            return MockResponse(200, "get_user1.json.resp")
        if url == "https://example.com/api/v1/users/user2@example.com" and method == "GET":
        ...
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S2/lifecycle/deactivate":
            return MockResponse(404)

        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S1/lifecycle/suspend":
            return MockResponse(200, "suspend_user_success.json.resp")
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S2/lifecycle/suspend":
            return MockResponse(404)
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/lifecycle/suspend":
            return MockResponse(400)

        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/lifecycle/unsuspend":
            return MockResponse(200, "unsuspend_user_success.json.resp")
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S2/lifecycle/unsuspend":
            return MockResponse(404)
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S1/lifecycle/unsuspend":
            return MockResponse(400)

        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/factors/aaa1qwerty0UIOPaS0d9/verify":
            return MockResponse(404)
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S2/factors/aaa1qwerty0UIOPaS0d1/verify":
            return MockResponse(404)

        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/factors/aaa1qwerty0UIOPaS0d1/verify":
            return MockResponse(201, "send_push_verify_challenge1.json.resp")
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/factors/aaa1qwerty0UIOPaS0d2/verify":
            return MockResponse(201, "send_push_verify_challenge2.json.resp")
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/factors/aaa1qwerty0UIOPaS0d3/verify":
            return MockResponse(201, "send_push_verify_challenge3.json.resp")
        if url == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/factors/aaa1qwerty0UIOPaS0d8/verify":
            return MockResponse(200, "send_push_verify_challenge_not_push.json.resp")

        if (
            url
            == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/factors/aaa1qwerty0UIOPaS0d1/transactions/abcde.ABCDEFGH"
        ):
            return MockResponse(200, "send_push_verify_poll_status_success.json.resp")
        if (
            url
            == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/factors/aaa1qwerty0UIOPaS0d2/transactions/asdfg.QWERTYUI"
        ):
            return MockResponse(200, "send_push_verify_poll_status_rejected.json.resp")
        if (
            url
            == "https://example.com/api/v1/users/00u1m1qwertYUiopA0S3/factors/aaa1qwerty0UIOPaS0d3/transactions/mmmmm.AAAAAAAA"
        ):
