...
class Connection(komand.Connection):
    ...
    def connect(self, params):
        ...
        if not username:
            self.logger.info('Connect: Username not provided, using secret via "x-auth-token"')
            username = "x-auth-token"

        self.git_repository = GitRepository(repository_url, username, secret, self.logger)
