...
from komand_cisco_umbrella_investigate.actions.related.action import Related
from komand_cisco_umbrella_investigate.actions.related.schema import Input, Output
...
STUB_RESPONSE = Util.read_file_to_dict("expected/test_related.json.exp")


class TestRelated(TestCase):
    @patch("requests.request", side_effect=mock_request_200)
    def setUp(self, mock_connection: Mock) -> None:
        self.action = Util.default_connector(Related())

    @parameterized.expand(
        [
            ("example.com", STUB_RESPONSE),
            ("example2.com", {Output.RELATED: []}),
        ]
    )
    @patch("requests.get", side_effect=mock_request_200)
    def test_related_ok(self, input_domain: str, expected_response: List[str], mock_get: Mock) -> None:
        response = self.action.run({Input.DOMAIN: input_domain})
    ...
    def test_related_exception(self, mock_request: Mock, input_parameters: Dict[str, Any], exception: str) -> None:
