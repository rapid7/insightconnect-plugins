...
from komand_cisco_umbrella_investigate.actions.rr_history_ip.action import RrHistoryIp
from komand_cisco_umbrella_investigate.actions.rr_history_ip.schema import Input, Output
...
STUB_RESPONSE = Util.read_file_to_dict("expected/test_rr_history_ip.json.exp")


class TestRrHistoryIp(TestCase):
    @patch("requests.request", side_effect=mock_request_200)
    def setUp(self, mock_connection: Mock) -> None:
        self.action = Util.default_connector(RrHistoryIp())

    @parameterized.expand(
        [
            ("192.168.0.1", "", STUB_RESPONSE),
            ("192.168.0.1", "A", STUB_RESPONSE),
        ]
    )
    @patch("requests.get", side_effect=mock_request_200)
    def test_rr_history_ip_ok(
        self, input_domain: str, input_type: str, expected_response: List[str], mock_get: Mock
    ) -> None:
        response = self.action.run({Input.IP: input_domain, Input.TYPE: input_type})
    ...
    def test_rr_history_ip_exception(
