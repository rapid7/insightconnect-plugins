...
from komand_cisco_umbrella_investigate.actions.sample_artifacts.action import SampleArtifacts
from komand_cisco_umbrella_investigate.actions.sample_artifacts.schema import Input, Output
...
STUB_RESPONSE = Util.read_file_to_dict("expected/test_sample.artifacts.json.exp")


class TestSampleArtifacts(TestCase):
    @patch("requests.request", side_effect=mock_request_200)
    def setUp(self, mock_connection: Mock) -> None:
        self.action = Util.default_connector(SampleArtifacts())
    ...
    def test_sample_artifacts_ok(self, input_hash: str, expected_response: List[str], mock_get: Mock) -> None:
    ...
    def test_sample_artifacts_exception(
