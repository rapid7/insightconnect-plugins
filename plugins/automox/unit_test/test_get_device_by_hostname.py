...
class TestGetDeviceByHostname(TestCase):
    def setUp(self) -> None:
        self.action = Util.default_connector(GetDeviceByHostname())
        self.params = {Input.ORG_ID: ORG_ID, Input.HOSTNAME: "WinHostName"}
    ...
    @patch("requests.Session.request", side_effect=mock_request_200)
    def test_get_device_by_hostname_when_hostname_not_existing(self, mock: Mock) -> None:
        self.params[Input.HOSTNAME] = "NonExistingHostName"
        response = self.action.run(self.params)
