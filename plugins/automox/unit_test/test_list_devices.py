...
from icon_automox.actions.list_devices import ListDevices
from icon_automox.actions.list_devices.schema import Input, Output


class TestListDevices(TestCase):
    def setUp(self) -> None:
        self.action = Util.default_connector(ListDevices())
        self.params = {Input.ORG_ID: ORG_ID, Input.GROUP_ID: GROUP_ID}

    @patch("requests.Session.request", side_effect=mock_request_200)
    def test_list_devices_ok(self, mock: Mock) -> None:
    ...
    def test_list_devices_exception(self, mock_request: Mock, exception: str) -> None:
