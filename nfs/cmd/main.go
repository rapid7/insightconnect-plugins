...
import (
	...
	"github.com/rapid7/komand-plugins/nfs/actions"
	"github.com/rapid7/komand-plugins/nfs/connection"
	"github.com/rapid7/komand-plugins/nfs/server/http"
	"github.com/rapid7/komand-plugins/nfs/triggers"
...
func main() {
	...
	case httpServer.FullCommand():
		...
		fmt.Printf("Plugin nfs booting HTTP Daemon on port %d...", *port)
...
func runOneOffAction(mode string, b *message.BodyV1, isDebug bool) error {
	...
	switch b.Action {
	case "read":
		input := &actions.ReadInput{}
		if err = json.Unmarshal(b.Input, input); err != nil {
			return err
		}
		for _, err = range input.Validate(l) {
			log.Println(fmt.Sprintf("Error while validating ReadTriggerInput: %s", err.Error()))
		}
		if err != nil {
			return fmt.Errorf("Error while validating ReadTriggerInput. Check logs for details")
		}
		a := &actions.ReadAction{}
		runMethod := a.Run
		if mode == "test" {
			runMethod = a.Test
		}
	...
	case "write":
		input := &actions.WriteInput{}
		if err = json.Unmarshal(b.Input, input); err != nil {
			return err
		}
		for _, err = range input.Validate(l) {
			log.Println(fmt.Sprintf("Error while validating WriteTriggerInput: %s", err.Error()))
		}
		if err != nil {
			return fmt.Errorf("Error while validating WriteTriggerInput. Check logs for details")
		}
		a := &actions.WriteAction{}
		runMethod := a.Run
		if mode == "test" {
			runMethod = a.Test
		}
...
func runTriggerDaemon(ctx context.Context, mode string, b *message.BodyV1, isDebug bool) error {
	...
	case "file_changed":
		...
		input := &triggers.FileChangedTriggerInput{}
		if err := json.Unmarshal(b.Input, input); err != nil {
			return err
		}
		// Build the trigger
		t := triggers.NewFileChangedTrigger(d, b.Meta)
		...
		go t.ReadLoop(ctx, l)
		var ti interface{} = t
		if cl, ok := ti.(triggers.FileChangedTriggerCustomLooper); ok {
...
func printInfo() {
	...
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "file_changed", cli.Reset, "Reports any file changes within the given interval", cli.Reset)

	// Action Info
	result += fmt.Sprintf("\n")
	result += fmt.Sprintf("Actions (%s%d%s): \n", cli.Green, 2, cli.Reset)
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "read", cli.Reset, "Read the named file via NFS", cli.Reset)
	result += fmt.Sprintf("└── %s%s%s (%s%s)\n", cli.Green, "write", cli.Reset, "Write to the named file via NFS", cli.Reset)
...
func printSampleInput(action string) error {
	...
	case "read":
		m.Type = "action_start"
		actionBytes, err := json.Marshal(actions.ReadInput{})
		if err != nil {
			return err
		}
		b.Input = actionBytes
	case "write":
		m.Type = "action_start"
		actionBytes, err := json.Marshal(actions.WriteInput{})
		if err != nil {
			return err
		}
		b.Input = actionBytes
	case "file_changed":
		...
		triggerBytes, err := json.Marshal(triggers.FileChangedTriggerInput{})
		if err != nil {
			return err
		}
