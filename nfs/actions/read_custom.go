...
func (i *ReadInput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (o *ReadOutput) Validate(log plog.Logger) []error {
	errs := make([]error, 0)
...
func (a *ReadAction) Run(conn *connection.Connection, input *ReadInput, log plog.Logger) (*ReadOutput, error) {
	output := &ReadOutput{}
	baseFH := conn.BaseFileHandle
	path := input.FilePathname
	numLines := input.LinesToRead
	...
	lines, err := implementation.NFSRead(baseFH, path, numLines, timeout, &config)

	if err == nil {
		output.LinesRead = lines
	}
	return output, err
}
...
func (a *ReadAction) Test(conn *connection.Connection, input *ReadInput, log plog.Logger) (*ReadOutput, error) {
	output := &ReadOutput{}
	// Your code here
