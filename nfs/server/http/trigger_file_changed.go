...
// FileChangedTriggerHandler is a handler
type FileChangedTriggerHandler struct {
	c *connection.Cache
}

// NewFileChangedTriggerHandler returns a new FileChangedHandler
func NewFileChangedTriggerHandler(c *connection.Cache) *FileChangedTriggerHandler {
	return &FileChangedTriggerHandler{
		c: c,
	}
}

// ServeHTTP handles the requests
func (h *FileChangedTriggerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating FileChangedInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating FileChangedInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	trigger := triggers.NewFileChangedTrigger(nil, response.Meta)
	var output *triggers.FileChangedTriggerOutput
...
func (h *FileChangedTriggerHandler) replyWithError(response *message.TriggerEvent, w http.ResponseWriter, err error) {
