...
// ReadHandler is a handler
type ReadHandler struct {
	c *connection.Cache
}

// NewReadHandler returns a new ReadHandler
func NewReadHandler(c *connection.Cache) *ReadHandler {
	return &ReadHandler{
		c: c,
	}
}
...
func (h *ReadHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating ReadInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating ReadInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
...
func (h *ReadHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
