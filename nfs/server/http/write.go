...
// WriteHandler is a handler
type WriteHandler struct {
	c *connection.Cache
}

// NewWriteHandler returns a new WriteHandler
func NewWriteHandler(c *connection.Cache) *WriteHandler {
	return &WriteHandler{
		c: c,
	}
}
...
func (h *WriteHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, err = range input.Validate(l) {
		log.Println(fmt.Sprintf("Error while validating WriteInput: %s", err.Error()))
	}
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		h.replyWithError(response, w, fmt.Errorf("Error while validating WriteInput. Check logs for details"))
		return
	}
	cd := &connection.Data{}
	...
	action := actions.WriteAction{}
	var output *actions.WriteOutput
	if isTest {
...
func (h *WriteHandler) replyWithError(response *message.Response, w http.ResponseWriter, err error) {
