...
func NewServer(port int, c *connection.Cache) (*http.Server, error) {
	...
	// one for each action
	mux.Handle("/actions/read", NewReadHandler(c))
	mux.Handle("/actions/read/test", NewReadHandler(c))
	mux.Handle("/actions/write", NewWriteHandler(c))
	mux.Handle("/actions/write/test", NewWriteHandler(c))
	mux.Handle("/triggers/file_changed/test", NewFileChangedTriggerHandler(c))
